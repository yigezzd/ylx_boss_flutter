...
/// 滞销商品（无时间选择器，默认当月）
class UnsalablePage extends StatefulWidget {
  const UnsalablePage({super.key});
  @override
  State<UnsalablePage> createState() => _UnsalablePageState();
}

class _UnsalablePageState extends State<UnsalablePage> {
  ...
  String _sortField = 'zxday', _sortType = 'desc';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
            title: const Text('滞销商品', style: TextStyle(fontSize: 17)),
            centerTitle: true,
      ...
      child: RefreshIndicator(
        ...
        child: DataTable2(
          ...
          columns: [
            ...
            DataColumn2(
                fixedWidth: 115,
                label: _sortHeader('滞销天数', 'zxday'),
                onSort: (_, __) => _onSort('zxday', _sortType == 'asc' ? 'desc' : 'asc')),
            const DataColumn2(
                fixedWidth: 110,
                label: Padding(
            ...
            DataColumn2(
                ...
                onSort: (_, __) => _onSort('endqty', _sortType == 'asc' ? 'desc' : 'asc')),
            DataColumn2(
                fixedWidth: 140,
                label: _sortHeader('最后销售日期', 'outbilldate'),
                onSort: (_, __) => _onSort('outbilldate', _sortType == 'asc' ? 'desc' : 'asc')),
            DataColumn2(
                fixedWidth: 125,
                label: _sortHeader('最后销售量', 'outqty'),
                onSort: (_, __) => _onSort('outqty', _sortType == 'asc' ? 'desc' : 'asc')),
            DataColumn2(
                fixedWidth: 110,
                label: _sortHeader('日均销量', 'avgqty'),
                onSort: (_, __) => _onSort('avgqty', _sortType == 'asc' ? 'desc' : 'asc')),
            DataColumn2(
                fixedWidth: 110,
                label: _sortHeader('可销天数', 'onsaleday'),
                onSort: (_, __) => _onSort('onsaleday', _sortType == 'asc' ? 'desc' : 'asc')),
          ...
          rows: [
            ...
            if (_loading && (_page > 1 || !_hasLoadedOnce))
              DataRow(cells: [
                ...
                for (int j = 1; j < (_productCodeDisabledFlag != 2 ? 15 : 14); j++) DataCell.empty
              ]),
          ],
        ),
      ),
    ...
    return DataRow2(
      ...
      cells: [
        ...
        DataCell(Align(
            ...
            child: Padding(
                padding: const EdgeInsets.symmetric(horizontal: 8),
                child: Text(row['zxday']?.toString() ?? '', style: cs)))),
        DataCell(Padding(
            padding: const EdgeInsets.symmetric(horizontal: 8),
