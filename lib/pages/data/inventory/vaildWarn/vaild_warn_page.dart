...
/// 有效期预警（无时间选择器，默认当月）
class VaildWarnPage extends StatefulWidget {
  const VaildWarnPage({super.key});
  @override
  State<VaildWarnPage> createState() => _VaildWarnPageState();
}

class _VaildWarnPageState extends State<VaildWarnPage> {
  ...
  // ── 筛选弹窗参数 ──
  int _days = 7; // 临期天数（默认7，0=全部）
  String _itemstatus = ''; // 商品状态（单选，空=全部）
  String _brandid = '', _brandname = ''; // 品牌
  ...
  Map<String, dynamic> _baseParams() => {
        ...
        if (_days != 0) 'datediffs': _days,
        if (_itemstatus.isNotEmpty) 'itemstatus': _itemstatus,
  ...
  Future<void> _loadData() async {
    ...
    try {
      final r = await request(HttpApi.stockwarnGetValidDateWarn,
  ...
  // ── 品牌选择（像选择供应商一样，使用 CommonSelectSheet 单选） ──
  Future<void> _selectBrand(
    ...
    final result = await CommonSelectSheet.show(
      ...
      title: '选择品牌',
      searchHint: '输入品牌名称',
      fetchData: (text, page) async {
        final params = <String, dynamic>{
          'is_page': 0,
          'stopflag': '0,3',
          if (text.isNotEmpty) 'cond': text,
          'sids': _sids,
        };
        final r = await request(HttpApi.brandList, params);
        final data = r['data'];
        if (data is List) {
          return {'list': data};
      ...
      showAll: true,
      initialSelectedId: _brandid.isNotEmpty ? _brandid : null,
      mapResult: (item) {
        final name = item['name']?.toString() ?? '';
        return {'id': item['id']?.toString() ?? '', 'name': name};
      },
    );
  ...
  void _openFilterSheet() {
    final initial = InventoryFilterData(
        ...
        itemstatus: _itemstatus.isNotEmpty ? [_itemstatus] : []);
        ...
        builder: (_) => InventoryFilterSheet(
            ...
            onConfirm: (data) {
              setState(() {
                ...
                _itemstatus = data.itemstatus.isEmpty ? '' : data.itemstatus.first;
              });
              // 同步页面搜索框（暂停 listener，避免 debounce 重复刷新）
              _condController.removeListener(_onCondChanged);
              _condController.text = data.cond;
              _condController.addListener(_onCondChanged);
              _resetAndLoad();
            }));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
            title: const Text('有效期预警', style: TextStyle(fontSize: 17)),
            centerTitle: true,
        ...
        body: Column(
            // 页面搜索框暂时屏蔽（筛选弹窗内已含搜索框），恢复时加回 _buildSearchBox()
            children: [_buildHeader(), Expanded(child: _buildCardList())]));
  }
  ...
  // TODO: 页面搜索框暂时屏蔽（筛选弹窗内已含搜索框），恢复时取消注释并加回 body 调用
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(mainAxisSize: MainAxisSize.min, children: [
              const Text('临期天数：', style: TextStyle(fontSize: 15, color: Color(0xFF333333))),
              Text(item['diffday']?.toString() ?? '0',
                  style: const TextStyle(
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('当前库存：${_checkstockflag == 1 ? (item['qty']?.toString() ?? '0') : '****'}',
                style: const TextStyle(fontSize: 13, color: Color(0xFF7A7A7A))),
            Text('生产批次：${item['batchno']?.toString() ?? '无'}',
                style: const TextStyle(fontSize: 13, color: Color(0xFF7A7A7A))),
          ]),
          // 第四行：到期日期
          const SizedBox(height: 10),
          Text('到期日期：${item['validdate']?.toString() ?? '无'}',
              style: const TextStyle(fontSize: 13, color: Color(0xFF7A7A7A))),
        ]),
      ),
    );
