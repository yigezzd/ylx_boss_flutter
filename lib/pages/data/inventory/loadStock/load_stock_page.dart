...
class _LoadStockPageState extends State<LoadStockPage> {
  ...
  int _days = 3;
  String _sortField = 'negativestockqtyday', _sortType = 'desc';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
            title: const Text('负库存', style: TextStyle(fontSize: 17)),
            centerTitle: true,
      ...
      child: RefreshIndicator(
        ...
        child: DataTable2(
          ...
          columns: [
            ...
            DataColumn2(
                fixedWidth: 155,
                label: _sortHeader('负库存持续天数', 'negativestockqtyday'),
                onSort: (_, __) => _onSort('negativestockqtyday',
                    _sortField == 'negativestockqtyday' && _sortType == 'asc' ? 'desc' : 'asc')),
            const DataColumn2(
                fixedWidth: 110,
                label: Padding(
            ...
            DataColumn2(
                fixedWidth: 140,
                label: _sortHeader('库存转负日期', 'negativebilldate'),
                onSort: (_, __) => _onSort('negativebilldate',
                    _sortField == 'negativebilldate' && _sortType == 'asc' ? 'desc' : 'asc')),
            DataColumn2(
                fixedWidth: 140,
            ...
            DataColumn2(
                fixedWidth: 115,
                label: _sortHeader('最后入库量', 'inqty'),
                onSort: (_, __) =>
                    _onSort('inqty', _sortField == 'inqty' && _sortType == 'asc' ? 'desc' : 'asc')),
          ...
          rows: [
            ...
            if (_loading && (_page > 1 || !_hasLoadedOnce))
              DataRow(cells: [
                ...
                for (int j = 1; j < (_productCodeDisabledFlag != 2 ? 17 : 16); j++) DataCell.empty
              ]),
          ],
        ),
      ),
    ...
    return DataRow2(
      ...
      cells: [
        ...
        DataCell(Align(
            ...
            child: Padding(
                padding: const EdgeInsets.symmetric(horizontal: 8),
                child: Text(row['negativestockqtyday']?.toString() ?? '', style: cs)))),
        DataCell(Padding(
            padding: const EdgeInsets.symmetric(horizontal: 8),
        ...
        DataCell(Padding(
            padding: const EdgeInsets.symmetric(horizontal: 8),
            child: Text(row['negativebilldate']?.toString() ?? '', style: cs))),
        DataCell(Padding(
            padding: const EdgeInsets.symmetric(horizontal: 8),
        ...
        DataCell(Align(
            ...
            child: Padding(
                padding: const EdgeInsets.only(right: 8),
                child: Text(_fmtNum(row['inqty']), style: cs)))),
        if (showCode)
          DataCell(Padding(
              padding: const EdgeInsets.symmetric(horizontal: 8),
