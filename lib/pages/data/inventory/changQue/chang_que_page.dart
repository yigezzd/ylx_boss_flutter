...
/// 畅缺商品（无时间选择器，默认当月）
/// 参考 D:\VUE\ylx-boss\src\subs\data\inventory\changQue\changQue.vue
class ChangQuePage extends StatefulWidget {
  const ChangQuePage({super.key});
  @override
  State<ChangQuePage> createState() => _ChangQuePageState();
}

class _ChangQuePageState extends State<ChangQuePage> {
  ...
  int _mpbilltypeflag = 1;
  String _sortField = 'cxday', _sortType = 'desc';
  ...
  Map<String, dynamic> _baseParams() => {
        ...
        'mpbilltypeflag': _mpbilltypeflag,
      };
        ...
        builder: (_) => InventoryFilterSheet(
            ...
            onConfirm: (data) {
              setState(() {
                ...
                _mpbilltypeflag = data.mpbilltypeflag;
              });
              _resetAndLoad();
            }));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
          title: const Text('畅缺商品', style: TextStyle(fontSize: 17)),
          centerTitle: true,
      ...
      child: RefreshIndicator(
        ...
        child: DataTable2(
          ...
          columns: [
            ...
            DataColumn2(
                fixedWidth: 110,
                label: _sortHeader('可销天数', 'onsaleday'),
                onSort: (_, __) => _onSort(
                    'onsaleday', _sortField == 'onsaleday' && _sortType == 'asc' ? 'desc' : 'asc')),
            const DataColumn2(
                fixedWidth: 110,
                label: Padding(
            ...
            DataColumn2(
                fixedWidth: 110,
                label: _sortHeader('日结库存', 'endqty'),
                onSort: (_, __) => _onSort(
                    'endqty', _sortField == 'endqty' && _sortType == 'asc' ? 'desc' : 'asc')),
            DataColumn2(
                fixedWidth: 110,
            ...
            DataColumn2(
                fixedWidth: 140,
                label: _sortHeader('畅缺持续天数', 'cxday'),
                onSort: (_, __) =>
                    _onSort('cxday', _sortField == 'cxday' && _sortType == 'asc' ? 'desc' : 'asc')),
            DataColumn2(
                fixedWidth: 140,
                label: _sortHeader('最后入库日期', 'outbilldate'),
                onSort: (_, __) => _onSort('outbilldate',
                    _sortField == 'outbilldate' && _sortType == 'asc' ? 'desc' : 'asc')),
            DataColumn2(
                fixedWidth: 120,
                label: _sortHeader('最后入库量', 'outqty'),
                onSort: (_, __) => _onSort(
            ...
            const DataColumn2(
                fixedWidth: 100,
                label: Padding(
                    padding: EdgeInsets.symmetric(horizontal: 8), child: Text('是否可促销', style: hs))),
          ],
          rows: [
            ...
            if (_loading && (_page > 1 || !_hasLoadedOnce))
              DataRow(cells: [
                ...
                for (int j = 1; j < (_productCodeDisabledFlag != 2 ? 16 : 15); j++) DataCell.empty
              ]),
          ],
        ),
      ),
    ...
    return DataRow2(
      ...
      cells: [
        ...
        DataCell(Align(
            ...
            child: Padding(
                padding: const EdgeInsets.symmetric(horizontal: 8),
                child: Text(row['onsaleday']?.toString() ?? '', style: cs)))),
        DataCell(Padding(
            padding: const EdgeInsets.symmetric(horizontal: 8),
        ...
        DataCell(Align(
            ...
            child: Padding(
                ...
                child: Text(_checkstockflag == 1 ? _fmtNum(row['endqty']) : '****', style: cs)))),
        DataCell(Align(
            alignment: Alignment.centerRight,
            child: Padding(
                padding: const EdgeInsets.only(right: 8),
        ...
        DataCell(Align(
            ...
            child: Padding(
                padding: const EdgeInsets.symmetric(horizontal: 8),
                child: Text(row['cxday']?.toString() ?? '', style: cs)))),
        DataCell(Padding(
            padding: const EdgeInsets.symmetric(horizontal: 8),
        ...
        DataCell(Padding(
            padding: const EdgeInsets.symmetric(horizontal: 8),
            child: Text(row['iszx']?.toString() ?? '否', style: cs))),
      ],
    );
  }
