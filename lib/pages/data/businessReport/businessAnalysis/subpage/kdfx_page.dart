...
/// 客单分析
/// 参考 D:\VUE\ylx-boss\src\subs\data\businessReport\businessAnalysis\subpage\kdfx.vue
class KdfxPage extends StatefulWidget {
  const KdfxPage({super.key});
  @override
  State<KdfxPage> createState() => _KdfxPageState();
}

class _KdfxPageState extends State<KdfxPage> {
  String _storeName = '';
  List<int> _sids = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      final r = await request(HttpApi.bossJyfxGetKdfx, {
        'is_page': 0,
        'sids': _sids,
        'sidsname': _storeName,
  ...
  String get _chartLabel => _listType == 'billnum'
      ? '客流量'
      : _listType == 'billprice'
          ? '客单价'
          : '营业额';

  /// 客流量 1 位小数，客单价/营业额 2 位
  int get _tooltipDecimals => _listType == 'billnum' ? 1 : 2;

  String get _tooltipSuffix => '';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('客单分析', style: TextStyle(fontSize: 17)),
        centerTitle: true,
          ...
          if (_loading && _list.isEmpty && !_hasLoadedOnce)
            ...
            Container(
              ...
              child: Row(children: [
                Expanded(
                    child:
                        _buildMetricCard('客流量', _formatDecimal(1, _sumData['billnum']), 'billnum')),
                const SizedBox(width: 8),
                Expanded(
                    child: _buildMetricCard(
                        '客单价', _formatDecimal(3, _sumData['billprice']), 'billprice')),
                const SizedBox(width: 8),
                Expanded(
                    child: _buildMetricCard('营业额', _formatDecimal(3, _sumData['rramt']), 'rramt')),
              ]),
            ),
          ...
          columns: const [
            ...
            DataColumn2(
                ...
                label: Padding(
                    ...
                    child: Text('客流量', style: headerStyle))),
            DataColumn2(
                numeric: true,
                label: Padding(
                    padding: EdgeInsets.symmetric(horizontal: 12),
                    child: Text('客单价', style: headerStyle))),
            DataColumn2(
                numeric: true,
                label: Padding(
                    padding: EdgeInsets.symmetric(horizontal: 12),
                    child: Text('营业额', style: headerStyle))),
          ],
          rows: [
                  ...
                  cells: [
                    ...
                    DataCell(Align(
                        ...
                        child: Padding(
                            ...
                            child:
                                Text(_formatDecimal(1, _list[i]['billnum']), style: cellStyle)))),
                    DataCell(Align(
                        alignment: Alignment.centerRight,
                        child: Padding(
                            padding: const EdgeInsets.only(right: 12),
                            child:
                                Text(_formatDecimal(3, _list[i]['billprice']), style: cellStyle)))),
                    DataCell(Align(
                        alignment: Alignment.centerRight,
                        child: Padding(
                            padding: const EdgeInsets.only(right: 12),
            ...
            child: Row(children: [
              ...
              SizedBox(
                  ...
                  child: Container(
                      ...
                      padding: const EdgeInsets.only(right: 12),
                      decoration: const BoxDecoration(border: Border(right: borderSide)),
                      child: Text(_formatDecimal(1, sum['billnum']), style: boldStyle))),
              SizedBox(
                  width: colW,
                  child: Container(
                      alignment: Alignment.centerRight,
                      padding: const EdgeInsets.only(right: 12),
                      decoration: const BoxDecoration(border: Border(right: borderSide)),
                      child: Text(_formatDecimal(3, sum['billprice']), style: boldStyle))),
              SizedBox(
                  width: colW,
                  child: Container(
                      alignment: Alignment.centerRight,
