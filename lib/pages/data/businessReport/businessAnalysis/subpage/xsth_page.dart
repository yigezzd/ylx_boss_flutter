...
/// 销售退货
/// 参考 D:\VUE\ylx-boss\src\subs\data\businessReport\businessAnalysis\subpage\xsth.vue
class XsthPage extends StatefulWidget {
  const XsthPage({super.key});
  @override
  State<XsthPage> createState() => _XsthPageState();
}

class _XsthPageState extends State<XsthPage> {
  String _storeName = '';
  List<int> _sids = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      final r = await request(HttpApi.bossJyfxGetXsth, {
        'is_page': 1,
        'page': _page,
        'pagesize': 20,
        'sids': _sids,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('销售退货', style: TextStyle(fontSize: 17)),
        centerTitle: true,
      ...
      body: Column(children: [
        ...
        Container(
            ...
            child: Row(children: [
              Expanded(child: _buildMetricCard('退货数量', _fmtNum(1, _sumData['rtnqty']))),
              const SizedBox(width: 8),
              Expanded(child: _buildMetricCard('退货金额', _fmtNum(3, _sumData['rtnamt']))),
            ])),
            ...
            columns: const [
              ...
              DataColumn2(
                  ...
                  label: Padding(
                      ...
                      child: Text('退货数量', style: headerStyle))),
              DataColumn2(
                  numeric: true,
                  label: Padding(
                      padding: EdgeInsets.symmetric(horizontal: 12),
                      child: Text('退货金额', style: headerStyle))),
            ],
            rows: [
    ...
    return [
      ...
      DataCell(Align(
          ...
          child: Padding(
              ...
              child: Text(_fmtNum(1, row['rtnqty']), style: cs)))),
      DataCell(Align(
          alignment: Alignment.centerRight,
          child: Padding(
              padding: const EdgeInsets.only(right: 12),
              child: Text(_fmtNum(3, row['rtnamt']), style: cs)))),
    ];
  }
}
