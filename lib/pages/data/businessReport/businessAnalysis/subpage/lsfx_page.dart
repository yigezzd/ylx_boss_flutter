...
/// 零售分析
/// 参考 D:\VUE\ylx-boss\src\subs\data\businessReport\businessAnalysis\subpage\lsfx.vue
class LsfxPage extends StatefulWidget {
  const LsfxPage({super.key});
  @override
  State<LsfxPage> createState() => _LsfxPageState();
}

class _LsfxPageState extends State<LsfxPage> {
  String _storeName = '';
  List<int> _sids = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      final r = await request(HttpApi.bossJyfxGetLsfx, {
        'is_page': 0,
        'sids': _sids,
        'sidsname': _storeName,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('零售分析', style: TextStyle(fontSize: 17)),
        centerTitle: true,
