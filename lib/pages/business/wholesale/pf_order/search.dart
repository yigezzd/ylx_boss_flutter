...
class PfOrderSearchPage extends StatefulWidget {
  const PfOrderSearchPage({super.key});

  @override
  State<PfOrderSearchPage> createState() => _PfOrderSearchPageState();
}

class _PfOrderSearchPageState extends State<PfOrderSearchPage> {
  int _page = 1;
  bool _loading = false;
  bool _hasMore = true;
  ...
  Future<void> _loadData() {
    ...
    return request(HttpApi.pfOrderFindList, {
      ...
      if (_dhflag.isNotEmpty) 'dhflag': _dhflag,
                      ...
                      child: Column(
                        ...
                        children: [
                            ...
                            children: [
                              Expanded(
                                child: _buildDateBtn(
                                  ...
                                  onTap: () async {
                                    final picked =
                                        await _pickDate(ctx, initial: tmpStart);
                                    if (picked != null) {
                              ...
                              Expanded(
                                child: _buildDateBtn(
                                  ...
                                  onTap: () async {
                                    final picked =
                                        await _pickDate(ctx, initial: tmpEnd);
                                    if (picked != null) {
                          ...
                          Wrap(
                            spacing: 10,
                            runSpacing: 8,
                            children: ['全部', '未完成', '部分完成']
                                .asMap()
                                .entries
                                .map((e) {
                              final values = ['', '1', '2'];
                              final active = tmpDhflag == values[e.key];
                              return GestureDetector(
                                onTap: () => setSheetState(
                                    () => tmpDhflag = values[e.key]),
                                child: Container(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: CupertinoPicker(
                        ...
                        itemExtent: 36,
                        onSelectedItemChanged: (i) {
                          final maxDay = DateTime(tempDate.year,
                                  tempDate.month + 1, 0)
                              .day;
                          tempDate = DateTime(
                              tempDate.year,
  ...
  String _dhflagLabel(String? dhflag) {
    if (dhflag == '1') return '未完成';
    if (dhflag == '2') return '部分完成';
    return '';
  }
    ...
    if (_isSelect) {
      ...
      if (billid != null && billid.isNotEmpty) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => PfOrderEditPage(billData: item),
          ),
        );
      }
    }
  ...
  Widget build(BuildContext context) {
    final title = _isSelect ? '选择批发订货单' : '搜索';
    return Scaffold(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        hintStyle: const TextStyle(
                            fontSize: 13, color: Color(0xFF8B8B8B)),
...
class _PfOrderSearchCard extends StatelessWidget {
  final Map<String, dynamic> item;
  final VoidCallback onTap;

  const _PfOrderSearchCard({required this.item, required this.onTap});

  static Color _dhflagBorderColor(String? dhflagname) {
    if (dhflagname == '未发货') return const Color(0xFFD9001B);
    if (dhflagname == '已发货') return const Color(0xFF2D8B0D);
    if (dhflagname == '部分发货') return const Color(0xFF006EFF);
    return const Color(0xFF999999);
  }

  static Color _dhflagTextColor(String? dhflagname) {
    return _dhflagBorderColor(dhflagname);
  }

  @override
  Widget build(BuildContext context) {
    final custname = item['custname']?.toString() ?? '-';
    final dhflagname = item['dhflagname']?.toString() ?? '';
    final signflag = item['signflag']?.toString() ?? '';
    final billno = item['billno']?.toString() ?? '-';
    final billamt = _formatDecimal(3, item['billamt']);
                        ...
                        if (dhflagname.isNotEmpty)
                          Container(
                            ...
                            child: Text(
                              ...
                              style: TextStyle(
                                fontSize: 11,
                                color: _dhflagTextColor(dhflagname),
                              ),
                            ),
                          ),
                      ],
