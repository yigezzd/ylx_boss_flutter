...
enum _PFAction { none, save, sign, delete, print, retsign, withdraw, generate }
...
void _writeData(String key, Map<String, dynamic> d) {
  ...
  switch (key) {
    case 'jsqty':
      ...
      d['zjsqty'] = _fd1(MathUtils.divide(_fd1(d['qty']), pn).floorToDouble());
      d['ssqty'] = _fd1(MathUtils.subtract(_fd1(d['qty']), MathUtils.mul(_fd1(d['zjsqty']), pn)));
      break;
    case 'presentqty':
    ...
    case 'qty':
      ...
      break;
    case 'zjsqty':
      d['zjsqty'] = _fd1(d['zjsqty']);
      d['qty'] = _fd1(MathUtils.add(MathUtils.mul(_fd1(d['zjsqty']), pn), _fd1(d['ssqty'])));
      break;
    case 'ssqty':
      d['ssqty'] = _fd1(d['ssqty']);
      if (pn <= _fd1(d['ssqty'])) d['ssqty'] = _fd1(pn - 1);
      d['qty'] = _fd1(MathUtils.add(MathUtils.mul(_fd1(d['zjsqty']), pn), _fd1(d['ssqty'])));
      break;
    case 'rate':
      d['rate'] = _fd1(d['rate']);
...
void _defValSet(Map<String, dynamic> d) {
  d['qty'] = _fd1(d['qty'] ?? 0);
  d['outtaxrate'] = _fd1(d['outtaxrate'] ?? 0);
  final pn = (_fd1(d['packagenum']) > 0 ? _fd1(d['packagenum']) : 1);
  d['jsqty'] = _fd1(MathUtils.divide(_fd1(d['qty']), pn));
...
class _PfOrderEditPageState extends State<PfOrderEditPage> with LogPageMixin<PfOrderEditPage> {
  @override
  String get logPageName => _isEdit ? '批发订货详情' : '批发订货新增';
      ...
      if (data is Map<String, dynamic>) {
        setState(() {
          ...
          for (final v in list) {
            ...
            row.rawData = c;
            if (row.rawData != null) _writeData('qty', row.rawData!);
            _items.add(row);
          }
        });
      }
  ...
  void _submit({bool doSign = false}) {
    ...
    request(HttpApi.pfOrderSave, params).then((result) {
      ...
      if (mounted && !doSign) setState(() => _submitAction = _PFAction.none);
    });
  }
  ...
  Future<void> _sign() async {
    if (!PermissionUtils.checkPermission('012205', showTip: false)) {
      Toast.show('你无权审核批发订货，请在后台修改权限');
      return;
    }
    ...
    if (_reviewFlowUsers.isNotEmpty) {
      _showApprovalDialog().then((approvalResult) {
        if (approvalResult != null && mounted) {
          ...
          setState(() => _submitAction = _PFAction.none);
        }
      });
      return;
    }
  ...
  /// 生成批发销售单（对齐 lxAss generateSaleOrder）
  Future<void> _generateSaleOrder() async {
    if (_billData == null) return;
    if (!PermissionUtils.checkPermission('012209', showTip: false)) {
      Toast.show('你无权生成批发单，请在后台修改权限');
      return;
    }
    final dhflag = _billData?['dhflag']?.toString();
    if (dhflag == '3') {
      Toast.show('该批发订货单已被引用，不能生成批发单！');
      return;
    }
    if (dhflag == '4') {
      Toast.show('该批发订货单已被终止，不能生成批发单！');
      return;
    }
    final confirm = await showDialog<bool>(
      context: context,
    ...
    if (confirm != true) return;
    setState(() => _submitAction = _PFAction.generate);
    request(HttpApi.pfOrderGeneratePfSell, [_billData]).then((result) {
      if (!mounted) return;
      Toast.show('生成成功');
      _loadDetail();
    }).whenComplete(() {
  ...
  Future<void> _restsign() async {
    if (_billData == null) return;
    if (!PermissionUtils.checkPermission('012206', showTip: false)) {
      Toast.show('你无权反审核批发订货，请在后台修改权限');
      return;
    }
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('提示'),
        content: const Text('撤回单据后，所有审批步骤需重新处理！是否撤回该批发订货单？'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('取消')),
          TextButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('确定')),
        ],
    ...
    if (confirm != true) return;
    setState(() => _submitAction = _PFAction.withdraw);
  ...
  void _doSign() {
    ...
    request(HttpApi.pfOrderSign, signParams).then((result) {
      if (!mounted) return;
      ...
      setState(() {
        ...
        for (final prod in result) {
          ...
          row.priceController.text =
              prod['pfprice1']?.toString() ?? prod['price']?.toString() ?? '0';
  ...
  Future<void> _handleScannedBarcode(String scanCode, {FocusNode? returnFocusNode}) async {
    ...
    // 价格优先级：custdiscountprice > 0 ? custdiscountprice : (custprice ?? price)
  ...
  double _getScanPrice(Map<String, dynamic> prod) {
    ...
    if (custprice != null) return custprice; // custprice=0 也是有效价格
    return double.tryParse(prod['price']?.toString() ?? '') ?? 0;
  }

  /// 按 barcode/productid/size/unit/supid/batchno 查找已在列表中的商品索引（对齐 lxAss）
  ...
  Future<Map<String, dynamic>?> _showExtendOptions(String type, Map<String, dynamic> raw) async {
    ...
    // size → name: size/sname, id: sizeonlyid/onlyid
    // unit → name: unit/sunit, id: unitonlyid/onlyid
            ...
            Flexible(
              child: ListView.builder(
                ...
                itemBuilder: (ctx, i) {
                  ...
                  final String code = item['scode']?.toString() ?? item['code']?.toString() ?? '';
                  return GestureDetector(
                    onTap: () {
                      ...
                      final sel = <String, dynamic>{
                        'unit': type == 'unit'
                            ? (item['unit']?.toString() ?? item['_name']?.toString() ?? '')
                            : (raw['unit']?.toString() ?? ''),
                        'unitonlyid': type == 'unit'
                            ? (item['_id']?.toString() ?? '')
                            : (raw['unitonlyid']?.toString() ?? ''),
                        'size': type == 'size'
                            ? (item['size']?.toString() ?? item['_name']?.toString() ?? '')
                            : (raw['size']?.toString() ?? ''),
                        'sizeonlyid': type == 'size'
                            ? (item['_id']?.toString() ?? '')
                            : (raw['sizeonlyid']?.toString() ?? ''),
                        'price': price,
                        'sellprice':
                            item['sellprice']?.toString() ?? item['retailprice']?.toString() ?? '',
                        'retailprice':
                            item['sellprice']?.toString() ?? item['retailprice']?.toString() ?? '',
                        'stockqty': item['stockqty']?.toString() ?? item['stock']?.toString() ?? '',
                        'barcode':
                            item['sbarcode']?.toString() ?? item['barcode']?.toString() ?? '',
                        ...
                        'pfprice1': item['pfprice1']?.toString() ?? '',
                        'pfprice2': item['pfprice2']?.toString() ?? '',
                        'pfprice3': item['pfprice3']?.toString() ?? '',
                        'mprice1': item['mprice1']?.toString() ?? '',
                        'mprice2': item['mprice2']?.toString() ?? '',
                        'mprice3': item['mprice3']?.toString() ?? '',
                        'psprice': item['psprice']?.toString() ?? '',
                        'packagenum': item['packagenum']?.toString() ?? '',
                      };
  ...
  Future<void> _showProDetailDrawer(_PFDetailRow row, int index) async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          _isEdit ? (_isSigned ? '批发订货详情' : '修改批发订货') : '新增批发订货',
  ...
  Widget _buildBottomBar() {
    ...
    // lxAss: 已审核 → [生成批发单] [打印] [反审]
    if (_readOnly) {
      return _buildBottomRow(
        buttons: [
          _BottomBtn(
              label: '生成批发单',
              onTap: _generateSaleOrder,
              perm: '012209',
              loading: _submitAction == _PFAction.generate),
          _BottomBtn(label: '打印', onTap: _print, perm: '012207'),
          _BottomBtn(label: '反审', onTap: _retsign, perm: '012206', primary: false),
        ],
      );
    }
    // lxAss: 新增 → [保存] [审核]
      ...
      return _buildBottomRow(
        buttons: [
          _BottomBtn(
              label: '保存',
              onTap: () {
                if (!PermissionUtils.checkPermission('012202', showTip: false)) {
                  Toast.show('你无权新增批发订货，请在后台修改权限');
                  return;
                }
                _submit();
              },
    ...
    if (_isSubmitted) {
      return _buildBottomRow(
        buttons: [
          _BottomBtn(label: '撤回', onTap: _restsign, perm: '012206', fillColor: const Color(0xFFFF9900)),
        ],
      );
    }
    ...
    return _buildBottomRow(
      buttons: [
        ...
        if (!_isWithdrawPending && _bolHandleTT)
          _BottomBtn(label: '更多', onTap: _showMoreSheet, perm: '012204', primary: false),
        ...
        if (!_isRejected &&
          ...
          _BottomBtn(
              label: '保存',
              onTap: () {
                if (!PermissionUtils.checkPermission('012203', showTip: false)) {
                  Toast.show('你无权编辑批发订货，请在后台修改权限');
                  return;
                }
                _submit();
              },
  ...
  /// 更多弹窗（删单 + 打印）
  void _showMoreSheet() {
    if (!PermissionUtils.checkPermission('012204', showTip: false)) {
      Toast.show('你无权删除批发订货，请在后台修改权限');
      return;
    }
        ...
        children: [
          Padding(
            ...
            child: Row(
              children: [
                Text(
                  '订货数量：$_totalOrderQty，共${_items.length}项，总金额：¥$_totalAmt',
...
class _PFDetailItemState extends State<_PFDetailItem> {
  ...
  void initState() {
    super.initState();
    _PFDetailRow._addOnFocusSelectAll(_priceFocus, _priceCtrl);
...
/// 粘性表头代理
class _PFStickyHeaderDelegate extends SliverPersistentHeaderDelegate {
