...
String _formatDecimal(int n, dynamic value) {
  final v = double.tryParse(value?.toString() ?? '0') ?? 0.0;
  return v.toStringAsFixed(n);
}

class PfOrderListPage extends StatefulWidget {
  const PfOrderListPage({super.key});

  @override
  State<PfOrderListPage> createState() => _PfOrderListPageState();
}

class _PfOrderListPageState extends State<PfOrderListPage>
    with SingleTickerProviderStateMixin, LogPageMixin<PfOrderListPage> {
  @override
  String get logPageName => '批发订货列表';
  ...
  void initState() {
    ...
    logEnter();
    if (!PermissionUtils.checkPermission('012201', showTip: false)) {
      Toast.show('你无权查看批发订货，请在后台修改权限');
      return;
    }
    _loadData();
  }
  ...
  Future<void> _loadData() {
    ...
    return request(HttpApi.pfOrderFindList, {
      'field': 'createtime',
      'type': 'desc',
      'page': _page,
                        ...
                        children: [
                          ...
                          Wrap(
                            ...
                            children: [
                              ...
                              const _FilterTag('未发货', '1'),
                              const _FilterTag('部分发货', '2'),
                              const _FilterTag('已发货', '3'),
                              const _FilterTag('已终止', '4'),
                            ].map((tag) {
                              final active = tmpDhflag == tag.value;
                              return GestureDetector(
                                onTap: () => setSheetState(() => tmpDhflag = tag.value),
                                child: Container(
                      ...
                      GestureDetector(
                        onTap: () async {
                          ...
                          await Navigator.push<bool>(
                            context,
                            MaterialPageRoute(
                              builder: (_) => const PfOrderEditPage(),
                            ),
                          );
                    ...
                    child: _PfOrderCard(
                      ...
                      onTap: () async {
                        ...
                        await Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => PfOrderEditPage(billData: _list[index]),
                          ),
                        );
                        if (mounted) _onRefresh();
                      },
...
/// 批发订货列表卡片（对齐 lxAss pfOrder/index.vue 列表项）
class _PfOrderCard extends StatelessWidget {
  const _PfOrderCard({required this.item, required this.onTap});
  ...
  static const Map<String, String> _dhflagLabelMap = {
    '0': '待处理',
    '1': '未发货',
    '2': '部分发货',
    '3': '已发货',
  ...
  static const Map<String, Color> _dhflagColorMap = {
    '0': Color(0xFF006EFF),
    '1': Color(0xFFD9001B),
    '2': Color(0xFF006EFF),
    '3': Color(0xFF2D8B0D),
  ...
  static String _dhflagLabel(String? dhflag) {
    return _dhflagLabelMap[dhflag] ?? '未发货';
  }

  static Color _dhflagColor(String? dhflag) {
    return _dhflagColorMap[dhflag] ?? const Color(0xFFD9001B);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final String billamt = _formatDecimal(3, item['billamt']);
    final String createtime = item['createtime']?.toString() ?? '-';
    final String signflag = item['signflag']?.toString() ?? '';
    final String dhflag = item['dhflag']?.toString() ?? '';
    final String dhflagname = item['dhflagname']?.toString() ?? _dhflagLabel(dhflag);
                      ...
                      children: [
                        ...
                        if (dhflagname.isNotEmpty)
                          Container(
                            ...
                            decoration: BoxDecoration(
                              border: Border.all(color: _dhflagColor(dhflag)),
                              borderRadius: BorderRadius.circular(4),
                            ),
                            child: Text(
                              dhflagname,
