...
class PfSaleSearchPage extends StatefulWidget {
  const PfSaleSearchPage({super.key});

  @override
  State<PfSaleSearchPage> createState() => _PfSaleSearchPageState();
}

class _PfSaleSearchPageState extends State<PfSaleSearchPage> {
  int _page = 1;
  bool _loading = false;
  bool _hasMore = true;
  ...
  Color _statusColor(String? signflag) {
    // 与 lxAss 小程序保持一致：已审核=灰色，待审核=蓝色
    if (signflag == '1') return const Color(0xFF404040);
    ...
    if (_isSelect) {
      ...
      if (billid != null && billid.isNotEmpty) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => PfSaleEditPage(billData: item),
          ),
        );
      }
    }
  ...
  Widget build(BuildContext context) {
    final title = _isSelect ? '选择批发销售单' : '搜索';
    return Scaffold(
...
class _PfSaleSearchCard extends StatelessWidget {
  final Map<String, dynamic> item;
  final VoidCallback onTap;

  const _PfSaleSearchCard({required this.item, required this.onTap});

  static String _fmtAmt(dynamic v) {
    final val = double.tryParse(v?.toString() ?? '0') ?? 0;
    // 与 lxAss formatDecimal(3) 保持一致
  ...
  Widget build(BuildContext context) {
    ...
    final billamt = _fmtAmt(item['billamt']);
    final storename = item['storename']?.toString() ?? '-';
    final countername = item['countername']?.toString() ?? '';
    final createname = item['createname']?.toString() ?? '-';
    final createtime = item['createtime']?.toString() ?? '-';
    final paystatusname = item['paystatusname']?.toString() ?? '';
                ...
                children: [
                  ...
                  if (paystatusname.isNotEmpty)
                    Container(
                      ...
                      decoration: BoxDecoration(
                        border: Border.all(
                            color: const Color(0xFF006EFF), width: 1),
