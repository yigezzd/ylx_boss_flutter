...
class PfSaleListPage extends StatefulWidget {
  const PfSaleListPage({super.key});

  @override
  State<PfSaleListPage> createState() => _PfSaleListPageState();
}

class _PfSaleListPageState extends State<PfSaleListPage>
    with SingleTickerProviderStateMixin, LogPageMixin<PfSaleListPage> {
  @override
  String get logPageName => '批发销售列表';
  ...
  void initState() {
    ...
    logEnter();
    if (!PermissionUtils.checkPermission('012301', showTip: false)) {
      Toast.show('你无权查看批发销售，请在后台修改权限');
      return;
    }
    _loadData();
  }
                      ...
                      GestureDetector(
                        onTap: () async {
                          ...
                          await Navigator.push<bool>(
                            context,
                            MaterialPageRoute(
                              builder: (_) => const PfSaleEditPage(),
                            ),
                          );
                    ...
                    child: _PfSaleCard(
                      ...
                      onTap: () async {
                        ...
                        await Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => PfSaleEditPage(billData: _list[index]),
                          ),
                        );
                        if (mounted) _onRefresh();
                      },
...
/// 批发销售列表卡片
class _PfSaleCard extends StatelessWidget {
  const _PfSaleCard({required this.item, required this.onTap});
