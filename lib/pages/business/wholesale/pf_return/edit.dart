...
enum _PFReturnAction { none, save, sign, delete, print, retsign, withdraw }
...
void _amtJudge(Map<String, dynamic> d) {
  ...
  d['grossrateamt'] = _fd3(MathUtils.subtract(amt, costpriceamt));
...
class PfReturnEditPage extends StatefulWidget {
  const PfReturnEditPage({super.key, this.billData});
  final Map<String, dynamic>? billData;

  @override
  State<PfReturnEditPage> createState() => _PfReturnEditPageState();
}

class _PfReturnEditPageState extends State<PfReturnEditPage> with LogPageMixin<PfReturnEditPage> {
  @override
  String get logPageName => _isEdit ? '批发退货详情' : '批发退货新增';
  ...
  _PFReturnAction _submitAction = _PFReturnAction.none;
  ...
  final List<_PFReturnDetailRow> _items = [];
  ...
  void _submit({bool doSign = false}) {
    ...
    final List<_PFReturnDetailRow> submitItems;
    if (_isEdit) {
    ...
    setState(() => _submitAction = _PFReturnAction.save);
    ...
    request(HttpApi.pfSellSave, params).then((result) {
      ...
      if (mounted && !doSign) setState(() => _submitAction = _PFReturnAction.none);
    });
  }
  ...
  Future<void> _sign() async {
    if (!PermissionUtils.checkPermission('012405', showTip: false)) {
      Toast.show('你无权审核批发退货，请在后台修改权限');
      return;
    }
    ...
    if (_reviewFlowUsers.isNotEmpty) {
      _showApprovalDialog().then((approvalResult) {
        if (approvalResult != null && mounted) {
          ...
          setState(() => _submitAction = _PFReturnAction.none);
        }
      });
      return;
    }
  ...
  Future<void> _retsign() async {
    ...
    setState(() => _submitAction = _PFReturnAction.retsign);
    request(HttpApi.pfSellRetsign, _billData).then((result) {
      if (!mounted) return;
      Toast.show('反审成功');
      _loadDetail();
    }).whenComplete(() {
      if (mounted) setState(() => _submitAction = _PFReturnAction.none);
    });
  }
  ...
  Future<void> _delBill() async {
    ...
    setState(() => _submitAction = _PFReturnAction.delete);
  ...
  Future<void> _print() async {
    if (_billData == null) return;
    setState(() => _submitAction = _PFReturnAction.print);
  ...
  Future<void> _restsign() async {
    if (_billData == null) return;
    if (!PermissionUtils.checkPermission('012406', showTip: false)) {
      Toast.show('你无权反审核批发退货，请在后台修改权限');
      return;
    }
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('提示'),
        content: const Text('撤回单据后，所有审批步骤需重新处理！是否撤回该批发退货单？'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('取消')),
          TextButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('确定')),
        ],
    ...
    if (confirm != true) return;
    setState(() => _submitAction = _PFReturnAction.withdraw);
  ...
  void _doSign() {
    if (_billData == null) return;
    setState(() => _submitAction = _PFReturnAction.sign);
  ...
  Future<void> _selectRefBill() async {
    // 前置校验：参考 lxAss cgth/edit.vue openRefbilltype 逻辑
    if (_storeid == null) {
      Toast.show('请先选择机构');
      return;
    }
    ...
    final result = await Navigator.push<Map<String, dynamic>>(
      context,
      MaterialPageRoute(
        builder: (_) => const PfSaleSearchPage(),
  ...
  void _loadRefBillDetails(Map<String, dynamic> refBill) {
    ...
    request(HttpApi.pfSellGetInfo, refBill).then((result) {
      if (!mounted) return;
      final data = result['data'];
      if (data is Map<String, dynamic>) {
        // 对齐 lxAss orderConfirm：拷贝客户信息
        ...
        setState(() {
          ...
          for (final v in list) {
            ...
            // 对齐 lxAss orderConfirm：引用销售单数量调整
            // qty = 未退量，presentqty = 未退赠品量
            row.rawData!['orderedqty'] =
                double.tryParse(c['incompleteQty']?.toString() ?? '0') ?? 0;
            row.rawData!['presentqty'] =
                double.tryParse(c['notpresentreturnedqty']?.toString() ?? '0') ?? 0;
            row.rawData!['qty'] = double.tryParse(c['notreturnedqty']?.toString() ?? '0') ?? 0;
            row.rawData!['returnqty'] =
                double.tryParse(c['notreturnedqty']?.toString() ?? '0') ?? 0;
  ...
  Future<void> _showProDetailDrawer(_PFReturnDetailRow row, int index) async {
  ...
  Future<Map<String, dynamic>?> _showExtendOptions(String type, Map<String, dynamic> raw) async {
    ...
    // lxAss pfReturn selectUnitFn: price = custprice
    // lxAss pfReturn selectSizeFn: price = sellprice
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          _isEdit ? (_isSigned ? '批发退货详情' : '修改批发退货') : '新增批发退货',
          ...
          child: CustomScrollView(
            ...
            slivers: [
              ...
              SliverPersistentHeader(
                pinned: true,
                delegate: _PFReturnStickyHeaderDelegate(state: this),
              ),
              if (_items.isNotEmpty)
                SliverList.builder(
    ...
    return Column(
      children: [
        ...
        if (_refbillnoController.text.isNotEmpty) ...[
          const Divider(height: 1, color: Color(0xFFF3F4F6)),
          _buildReadonlyField(label: '批发销售单', value: _refbillnoController.text),
        ],
        const Divider(height: 1, color: Color(0xFFF3F4F6)),
  ...
  Widget _buildBottomBar() {
    ...
    if (_readOnly) {
      return _buildBottomRow(
        buttons: [
          _BottomBtn(label: '打印', onTap: _print, perm: '012407'),
          _BottomBtn(label: '反审', onTap: _retsign, perm: '012406', primary: false),
        ],
      );
    }
    // lxAss: 新增 → [保存] [审核]
      ...
      return _buildBottomRow(
        buttons: [
          _BottomBtn(
              ...
              loading: _submitAction == _PFReturnAction.save),
          // 审核按钮 - billSign（保存并审核）
          if (_billSign)
            _BottomBtn(
                label: '审核',
                ...
                loading: _submitAction == _PFReturnAction.sign),
        ],
      );
    }
    ...
    if (_isSubmitted) {
      return _buildBottomRow(
        buttons: [
          _BottomBtn(label: '撤回', onTap: _restsign, perm: '012406', fillColor: const Color(0xFFFF9900)),
        ],
      );
    }
    ...
    return _buildBottomRow(
      buttons: [
        ...
        if (!_isWithdrawPending && _bolHandleTT)
          _BottomBtn(label: '更多', onTap: _showMoreSheet, perm: '012404', primary: false),
        ...
        if (!_isRejected &&
          ...
          _BottomBtn(
              label: '保存',
              onTap: () {
                if (!PermissionUtils.checkPermission('012403', showTip: false)) {
                  Toast.show('你无权编辑批发退货，请在后台修改权限');
                  return;
                }
                _submit();
              },
  ...
  /// 更多弹窗（删单 + 打印）
  void _showMoreSheet() {
    if (!PermissionUtils.checkPermission('012404', showTip: false)) {
      Toast.show('你无权删除批发退货，请在后台修改权限');
      return;
    }
...
/// 明细行 Widget
class _PFReturnDetailItem extends StatefulWidget {
  const _PFReturnDetailItem({
    required this.row,
    required this.index,
  ...
  @override
  State<_PFReturnDetailItem> createState() => _PFReturnDetailItemState();
}

class _PFReturnDetailItemState extends State<_PFReturnDetailItem> {
  ...
  void initState() {
    super.initState();
    _PFReturnDetailRow._addOnFocusSelectAll(_priceFocus, _priceCtrl);
    _PFReturnDetailRow._addOnFocusSelectAll(_giftFocus, _giftCtrl);
    _PFReturnDetailRow._addOnFocusSelectAll(_qtyFocus, _qtyCtrl);
  }
  ...
  void _notifyChange() {
    ...
    widget.row.amt = _fd3(widget.row.rawData?['amt']);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final String unitStr =
        widget.row.unit.isNotEmpty ? widget.row.unit : (raw?['unit']?.toString() ?? '');
                    ...
                    children: [
                      Text(
                        () {
                          ...
                          if (widget.row.size.isNotEmpty) buf.write('/${widget.row.size}');
                          if (unitStr.isNotEmpty) buf.write('（$unitStr）');
                          return buf.toString();
                        }(),
                        style: const TextStyle(
                  ...
                  children: [
                    ...
                    ListenableBuilder(
                      ...
                      builder: (_, __) {
                        ...
                        final a = MathUtils.formatDecimal(3, q * p);
                        return Padding(
...
/// 粘性表头代理
class _PFReturnStickyHeaderDelegate extends SliverPersistentHeaderDelegate {
  _PFReturnStickyHeaderDelegate({required this.state});
  final _PfReturnEditPageState state;
