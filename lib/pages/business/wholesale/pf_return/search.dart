...
class PfReturnSearchPage extends StatefulWidget {
  const PfReturnSearchPage({super.key});

  @override
  State<PfReturnSearchPage> createState() => _PfReturnSearchPageState();
}

class _PfReturnSearchPageState extends State<PfReturnSearchPage> {
  int _page = 1;
  bool _loading = false;
  bool _hasMore = true;
    ...
    if (_isSelect) {
      ...
      if (billid != null && billid.isNotEmpty) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => PfReturnEditPage(billData: item),
          ),
        );
      }
    }
  ...
  Widget build(BuildContext context) {
    final title = _isSelect ? '选择批发退货单' : '搜索';
    return Scaffold(
...
class _PfReturnSearchCard extends StatelessWidget {
  final Map<String, dynamic> item;
  final VoidCallback onTap;

  const _PfReturnSearchCard({required this.item, required this.onTap});
