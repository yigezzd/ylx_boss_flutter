...
/// 客户结算单列表页 —— 对齐 boss 项目 custPlierpay.vue
class CustomerPayListPage extends StatefulWidget {
  const CustomerPayListPage({super.key});

  @override
  State<CustomerPayListPage> createState() => _CustomerPayListPageState();
}

class _CustomerPayListPageState extends State<CustomerPayListPage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (!PermissionUtils.checkPermission('014501', showTip: false)) {
      Toast.show('你无权查看客户结算，请在后台修改权限');
      return;
    }
    _loadReviewConfig();
  }
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('客户结算单');
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('客户结算单', force: true);
    if (!mounted) return;
    _applyTabCount(show ? 4 : 3);
  }
  ...
  Future<void> _selectStore() async {
    // 对齐 Vue custPlierpay.vue：登录门店 custflag==1 时传 custflag: 1，且不传 storetypes
  ...
  Future<void> _loadData() {
    ...
    return request(HttpApi.financeCustomerPayGetList, {
      'is_page': 1,
      'field': 'createtime',
      'type': 'desc',
      'cond': '',
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _FilterSelectRow(
                              label: '客户',
                              value: tmpCustname.isEmpty ? '全部客户' : tmpCustname,
                              onTap: () => _openCustSelector(ctx, setSheetState,
                        ...
                        return RepaintBoundary(
                          child: _CustomerPayCard(
                            ...
                            onTap: () {
                              ...
                              NavigatorUtils.pushResult(
                                context,
                                '${FinanceRouter.customerPayEdit}?billid=$billid',
                                // 对齐采购入库：返回后重评审批配置（已驳回 tab 及时出现/消失）
                                (_) => _onRefresh(),
                              );
                            },
  ...
  Future<void> _openCustSelector(
    ...
    final result = await CommonSelectSheet.show(
      ...
      fetchData: (search, page) => request(HttpApi.customerFindList, {
        'cond': search,
        'is_page': 1,
        'page': page,
        'pagesize': 20,
...
class _CustomerPayCard extends StatelessWidget {
  const _CustomerPayCard({required this.item, required this.onTap});
