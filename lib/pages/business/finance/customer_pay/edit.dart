...
/// 客户结算单编辑页 —— 对齐 boss 项目 custPlierpayEdit.vue
class CustomerPayEditPage extends StatefulWidget {
  const CustomerPayEditPage({super.key, this.billid = ''});

  /// 单据ID，为空表示新增
  final String billid;

  @override
  State<CustomerPayEditPage> createState() => _CustomerPayEditPageState();
}

class _CustomerPayEditPageState extends State<CustomerPayEditPage> {
  // ── 状态 ──
  bool _loading = false;
  ...
  List<Map<String, dynamic>> _custpayflow = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      final res = await request(HttpApi.financeCustomerPayGetInfo, {'billid': billid});
      final data = res['data'];
      if (data is Map<String, dynamic> && mounted) {
        setState(() {
          _formData = Map<String, dynamic>.from(data);
          // 提取 custpayflow
          final flowList = data['custpayflow'] as List? ?? [];
          _custpayflow = flowList
              .whereType<Map<dynamic, dynamic>>()
              .map((e) => Map<String, dynamic>.from(e))
              .toList();
          // 同步表单字段
          _custid = _str(data['custid']);
          _custname = _str(data['custname']);
          ...
          // 初始化每条明细
          for (int i = 0; i < _custpayflow.length; i++) {
            _initBillInfo(_custpayflow[i], i);
          }
          // 汇总
          final sumData = _sumFields(_custpayflow, [
            'billamt',
            'paidamt',
            'freetotalamt',
            'prepaidtotalamt',
  ...
  Future<void> _selectCustomer() async {
    ...
    if (result != null && mounted) {
      setState(() {
        ...
        _custname = result['custname']?.toString() ?? '';
      });
      if (_custid.isNotEmpty && _bsid.isNotEmpty) _fetchCustomerBill();
    }
  }

  Future<void> _selectStore() async {
    // 对齐 Vue custPlierpayEdit.vue：stopflag: 0 + custflag==1 时传 custflag: 1
    final custflag = SelectStorePage.loginStoreField('custflag') == '1' ? '1' : null;
      ...
      builder: (ctx) => !isNew
          ...
          : SafeArea(
              child: Container(
                ...
                child: _CustomerSelectBillSheet(
                  bills: _allBills,
                  selectedIds: _custpayflow.map((e) => e['custbillid']?.toString() ?? '').toSet(),
                  advance: _advance,
                  bsid: _bsid,
                  custid: _custid,
  ...
  Future<void> _pickPaytime() async {
    final now = DateTime.now();
    final initial = _paytime.isNotEmpty
        ? _paytime
        : '${_fmtDate(now)} ${now.hour.toString().padLeft(2, '0')}:${now.minute.toString().padLeft(2, '0')}:${now.second.toString().padLeft(2, '0')}';
  ...
  Future<void> _fetchCustomerBill() async {
    _allBills = [];
    setState(() => _billLoading = true);
    try {
      final res = await request(
          HttpApi.financeCustomerPayGetCustomerBill,
          {
            'is_page': 0,
            'bsid': _bsid,
      ...
      if (data is List && mounted) {
        ...
        setState(() {
          ...
          _billnum = sumData['billnum']?.toString() ?? '';
          _billamt = sumData['billamt']?.toString() ?? '';
          // 新增模式：对齐 Vue getCustomerBill 后 sumFields 覆盖（付款金额=本次应付合计、
  ...
  void _settleConfirm(String type) {
    ...
    for (final item in _custpayflow) {
      item['payamt'] = 0;
      item['freeamt'] = 0;
      item['prepaidamt'] = 0;
  ...
  void _comBillList(List<Map<String, dynamic>> arr, dynamic money) {
    ...
    for (final item in arr) {
      ...
      item['custbillid'] = item['custbillid'] ?? item['billid'];
      item['custbillno'] = item['custbillno'] ?? item['billno'];
      item['custbilldate'] = item['custbilldate'] ?? item['billdate'];
  ...
  void _initBillInfo(Map<String, dynamic> obj, int index) {
    ...
    obj['custbillid'] = obj['custbillid'] ?? obj['billid'];
    obj['custbillno'] = obj['custbillno'] ?? obj['billno'];
    obj['custbilldate'] = obj['custbilldate'] ?? obj['billdate'];
    obj['isort'] = index + 1;
  }
  ...
  Future<void> _handleSave([int sign = 0]) async {
    if (_isEdit) {
      if (!PermissionUtils.checkPermission('014503', showTip: false)) {
        Toast.show('你无权编辑客户结算，请在后台修改权限');
        return;
      }
    } else {
      if (!PermissionUtils.checkPermission('014502', showTip: false)) {
        Toast.show('你无权新增客户结算，请在后台修改权限');
        return;
      }
    }
    if (_saving) return;
    if (_custpayflow.isEmpty) {
      Toast.show('请选择单据');
      return;
    }
    if (_bsid.isEmpty) {
    ...
    try {
      ...
      final api = isNew ? HttpApi.financeCustomerPayAdd : HttpApi.financeCustomerPayUpdate;
  ...
  Future<void> _handleSign() async {
    if (!PermissionUtils.checkPermission('014505', showTip: false)) {
      Toast.show('你无权审核客户结算，请在后台修改权限');
      return;
    }
  ...
  void _doSign({bool isWithdraw = false}) {
    ...
    setState(() => _saving = true);
    request(HttpApi.financeCustomerPaySign, params).then((_) {
      if (mounted) {
        Toast.show('审核成功');
        _loadData();
      }
  ...
  Future<void> _handleZf() async {
    if (!PermissionUtils.checkPermission('014506', showTip: false)) {
      Toast.show('你无权作废客户结算，请在后台修改权限');
      return;
    }
    final confirm = await showDialog<bool>(
      context: context,
    ...
    setState(() => _saving = true);
    try {
      await request(HttpApi.financeCustomerPayZf, _formData);
      if (mounted) {
        Toast.show('作废成功');
  ...
  Future<void> _handleDelete() async {
    if (!PermissionUtils.checkPermission('014504', showTip: false)) {
      Toast.show('你无权删除客户结算，请在后台修改权限');
      return;
    }
    final confirm = await showDialog<bool>(
      context: context,
    ...
    try {
      final currentBillid =
          widget.billid.isNotEmpty ? widget.billid : (_formData['billid']?.toString() ?? '');
      await request(HttpApi.financeCustomerPayDel, {'billid': currentBillid});
      if (mounted) {
        Toast.show('删除成功');
  ...
  Future<void> _handleRetsign() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('提示'),
        content: const Text('撤回单据后，所有审批步骤需重新处理！是否撤回该客户结算单？'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('取消')),
          TextButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('确定')),
        ],
  ...
  void _handlePrint() {
    request(HttpApi.cgorderPrint, {'menuid': '101701', 'data': _formData}).then((_) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        centerTitle: true,
        title: Text(_isEdit ? '客户结算单详情' : '新增客户结算单',
      ...
      body: Stack(children: [
        SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 8),
            _buildFormCard([
              _buildSelectRow('客户', _custname,
                  onTap: !_isReadonly && !_isEdit ? _selectCustomer : null, required: true),
              _buildSelectRow('机构', _bstorename,
                  onTap: !_isReadonly && !_isEdit ? _selectStore : null, required: true),
              _buildSelectRow('选择单据', _custpayflow.isNotEmpty ? '单据数量：$_billnum，总金额：$_billamt' : '',
            ...
            Container(
              ...
              child: Column(children: [
                ...
                _buildSelectRow('收款方式', _paywayname,
                    onTap: !_isReadonly ? _selectPayway : null, required: true),
                _buildSelectRow('收款日期', _paytime, onTap: !_isReadonly ? _pickPaytime : null),
    ...
    if (showSaveAndAudit) {
      ...
      buttons.add(Expanded(
        ...
        child: Container(
          ...
          margin: const EdgeInsets.symmetric(horizontal: 4),
          decoration:
              BoxDecoration(color: const Color(0xFF006EFF), borderRadius: BorderRadius.circular(8)),
          alignment: Alignment.center,
          child: const Text('审核',
              style: TextStyle(fontSize: 15, color: Colors.white, fontWeight: FontWeight.w500)),
        ),
    ...
    return SafeArea(
      ...
      child: Column(children: [
        ...
        Expanded(
          ...
          itemCount: _custpayflow.length,
          itemBuilder: (_, i) {
            final bill = _custpayflow[i];
            final typeName = _CustomerSelectBillSheetState._billTypeName(bill['billtype']);
            final createtime =
                bill['createtime']?.toString() ?? bill['custbilldate']?.toString() ?? '';
            return RepaintBoundary(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Flexible(
                      child: Text(bill['custbillno']?.toString() ?? '',
                          maxLines: 1,
                ...
                Expanded(
                  ...
                  onPressed: () {
                    ...
                    Navigator.pop(context,
                        {'reviewsignflag': _flag, 'reviewremark': _remarkCtrl.text.trim()});
                  },
  ...
  Widget _buildRadio(int value) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color: isSelected ? const Color(0xFF006EFF) : const Color(0xFFCCCCCC), width: 2),
                  ...
                  return Container(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Row(children: [
                        ...
                        Text(item['signtime']?.toString() ?? item['createtime']?.toString() ?? '',
                            style: const TextStyle(fontSize: 12, color: Color(0xFF999999))),
                      ]),
...
// =================== 客户选择单据弹窗（对齐 Vue custPlierpay/selectBill.vue）==================
class _CustomerSelectBillSheet extends StatefulWidget {
  const _CustomerSelectBillSheet({
    required this.bills,
  ...
  @override
  State<_CustomerSelectBillSheet> createState() => _CustomerSelectBillSheetState();
}

class _CustomerSelectBillSheetState extends State<_CustomerSelectBillSheet> {
  /// 类型选项（对齐 Vue custPlierpay typeList：0期初金额/1批发销售/2批发退货/3客户费用/4客户补差）
  ...
  void initState() {
    ...
    _all = widget.bills
        .map((e) => Map<String, dynamic>.from(e)
          ..['_selected'] = _selectedIds.contains(e['custbillid']?.toString() ?? ''))
        .toList();
    ...
    _applyFilter();
    // 外层 getCustomerBill 请求仍在进行中：弹窗内自行重新拉取并显示 loading
  ...
  void _applyFilter() {
    ...
    final list = _all.where((e) {
      ...
      if (kw.isNotEmpty &&
          !(e['custbillno']?.toString() ?? '').contains(kw) &&
  ...
  void _toggle(Map<String, dynamic> bill) {
    final id = bill['custbillid']?.toString() ?? '';
    setState(() {
  ...
  void _toggleAll() {
    setState(() {
      ...
      for (final b in _filtered) {
        final id = b['custbillid']?.toString() ?? '';
    ...
    try {
      ...
      if (data is List) {
        ...
        for (final b in bills) {
          _selectedIds.add(b['custbillid']?.toString() ?? '');
          b['_selected'] = true;
        }
        _all = bills;
        _applyFilter();
  ...
  static void _recalcBill(Map<String, dynamic> item) {
    final nowamt = double.tryParse(item['nowamt']?.toString() ?? '') ?? 0;
    final result = _CustomerPayEditPageState._calculatePayment(nowamt, {
      'nowamt': nowamt,
      'freeamt': 0,
  ...
  /// 单据类型名称（对齐 Vue custPlierpay billNameFn：0期初金额/1批发销售/2批发退货/3客户费用/4客户补差）
  ...
  Widget _buildBillCard(Map<String, dynamic> bill) {
    ...
    final createtime = bill['createtime']?.toString() ?? bill['custbilldate']?.toString() ?? '';
    return RepaintBoundary(
        child: GestureDetector(
      onTap: () => _toggle(bill),
      child: Container(
