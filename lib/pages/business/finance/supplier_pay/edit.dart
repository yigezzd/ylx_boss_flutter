...
/// 供应商结算编辑页 —— 对齐 boss 项目 purchasePlierpayEdit.vue
class SupplierPayEditPage extends StatefulWidget {
  const SupplierPayEditPage({super.key, this.billid = ''});

  /// 单据ID，为空表示新增
  final String billid;

  @override
  State<SupplierPayEditPage> createState() => _SupplierPayEditPageState();
}

class _SupplierPayEditPageState extends State<SupplierPayEditPage> {
  // ── 状态 ──
  bool _loading = false;
  ...
  List<Map<String, dynamic>> _suppayflow = [];
  ...
  void initState() {
    ...
    // 对齐 Vue @blur="payamtBlur/freeamtBlur/prepaidamtBlur"：金额输入框失焦时结算并分配到明细
    _payamtFocus.addListener(() {
      if (!_payamtFocus.hasFocus && mounted && !_isReadonly) _settleConfirm('payamt');
    });
    _freeamtFocus.addListener(() {
      if (!_freeamtFocus.hasFocus && mounted && !_isReadonly) _settleConfirm('freeamt');
    });
    _prepaidamtFocus.addListener(() {
      if (!_prepaidamtFocus.hasFocus && mounted && !_isReadonly) _settleConfirm('prepaidamt');
    });
  }
  ...
  Future<void> _loadData() async {
    if (_loading) return;
    // 优先用 widget.billid，新增成功后从 _formData 取返回的 billid（对齐 Vue query.value = res 后 getInfo）
    final billid =
        widget.billid.isNotEmpty ? widget.billid : (_formData['billid']?.toString() ?? '');
    if (billid.isEmpty) return;
    setState(() => _loading = true);
    try {
      final res = await request(HttpApi.financeSupplierPayGetInfo, {'billid': billid});
      final data = res['data'];
      if (data is Map<String, dynamic> && mounted) {
        setState(() {
          _formData = Map<String, dynamic>.from(data);
          // 提取 suppayflow
          final flowList = data['suppayflow'] as List? ?? [];
          _suppayflow = flowList
              .whereType<Map<dynamic, dynamic>>()
              .map((e) => Map<String, dynamic>.from(e))
              .toList();
          // 同步表单字段
          _supid = _str(data['supid']);
          _supname = _str(data['supname']);
          ...
          // 初始化每条明细
          for (int i = 0; i < _suppayflow.length; i++) {
            _initBillInfo(_suppayflow[i], i);
          }
          // 汇总
          final sumData = _sumFields(_suppayflow, [
            'billamt',
            'paidamt',
            'freetotalamt',
            'prepaidtotalamt',
  ...
  Future<void> _selectSupplier() async {
    ...
    if (result != null && mounted) {
      ...
      if (_supid.isNotEmpty && _bsid.isNotEmpty) _fetchSupplierBill();
    }
  }

  Future<void> _selectStore() async {
    // 对齐 Vue purchasePlierpayEdit.vue：stopflag: 0 + supflag==1 时传 supflag: 1
    final supflag = SelectStorePage.loginStoreField('supflag') == '1' ? '1' : null;
      ...
      builder: (ctx) => isNew
          ? SafeArea(
              child: Container(
                ...
                child: _SelectBillSheet(
                  bills: _allBills,
                  selectedIds: _suppayflow.map((e) => e['supbillid']?.toString() ?? '').toSet(),
                  advance: _advance,
                  bsid: _bsid,
                  supid: _supid,
  ...
  Future<void> _fetchSupplierBill() async {
    _allBills = [];
    setState(() => _billLoading = true);
    try {
      final res = await request(
          HttpApi.financeSupplierPayGetSupplierBill,
          {
            'is_page': 0,
            'bsid': _bsid,
      ...
      if (data is List && mounted) {
        ...
        setState(() {
          ...
          _billnum = sumData['billnum']?.toString() ?? '';
          _billamt = sumData['billamt']?.toString() ?? '';
          // 新增模式：对齐 Vue getSupplierBill 后 sumFields 覆盖（付款金额=本次应付合计、
  ...
  void _comBillList(List<Map<String, dynamic>> arr, dynamic money) {
    ...
    for (final item in arr) {
      ...
      item['supbillid'] = item['supbillid'] ?? item['billid'];
      item['supbillno'] = item['supbillno'] ?? item['billno'];
      item['supbilldate'] = item['supbilldate'] ?? item['billdate'];
  ...
  void _initBillInfo(Map<String, dynamic> obj, int index) {
    ...
    obj['supbillid'] = obj['supbillid'] ?? obj['billid'];
    obj['supbillno'] = obj['supbillno'] ?? obj['billno'];
    obj['supbilldate'] = obj['supbilldate'] ?? obj['billdate'];
    obj['isort'] = index + 1;
  }
  ...
  Future<void> _handleSave([int sign = 0]) async {
    if (_isEdit) {
      if (!PermissionUtils.checkPermission('014402', showTip: false)) {
        Toast.show('你无权编辑供应商结算，请在后台修改权限');
        return;
      }
    } else {
      if (!PermissionUtils.checkPermission('014403', showTip: false)) {
        Toast.show('你无权新增供应商结算，请在后台修改权限');
        return;
      }
    }
    if (_saving) return;
    if (_suppayflow.isEmpty) {
      Toast.show('请选择单据');
      return;
    }
    if (_bsid.isEmpty) {
    ...
    try {
      final params = <String, dynamic>{
        ...
        'suppayflow': _suppayflow,
        'billnum': _billnum,
        'billamt': _billamt,
      };
      // billid 优先取 widget.billid（编辑模式），其次取 _formData（新增保存后的编辑模式）
      final currentBillid =
          widget.billid.isNotEmpty ? widget.billid : (_formData['billid']?.toString() ?? '');
      if (currentBillid.isNotEmpty) params['billid'] = currentBillid;
      // 审批流相关字段（撤回时 review signflag=2，审核时 reviewsignflag + reviewremark）
      ...
      // isNew: widget.billid 为空且 _formData 无 billid，才是真正的“新增”
      final isNew = currentBillid.isEmpty;
      final api = isNew ? HttpApi.financeSupplierPayAdd : HttpApi.financeSupplierPayUpdate;
      final res = await request(api, params);
      final data = res['data'];
      if (mounted) {
        Toast.show(res['retmsg']?.toString() ?? '保存成功');
        // 在 _loadData 刷新 _formData 之前，保存撤回意图（此时 _formData 还包含保存前的值）
        final bool isWithdraw = sign == 1 && _isWithdrawPending;
        if (data is Map<String, dynamic>) {
          // 新增/编辑保存后：更新表单数据和审批流（对齐 Vue query.value = res + getInfo）
          _formData = Map<String, dynamic>.from(data);
          _startdate = _str(data['startdate']).split(' ').first;
          _enddate = _str(data['enddate']).split(' ').first;
          setState(() {
  ...
  void _handleSignAfterSave(Map<String, dynamic>? savedData, {bool isWithdraw = false}) {
    ...
    // 撤回状态(reviewsignflag=2)：直接发 sign API，不弹审批弹窗，保留撤回标记
    if (isWithdraw) {
  ...
  Future<void> _handleSign() async {
    if (!PermissionUtils.checkPermission('014405', showTip: false)) {
      Toast.show('你无权审核供应商结算，请在后台修改权限');
      return;
    }
  ...
  void _doSign({bool isWithdraw = false}) {
    ...
    // 供应商结算审核：signflag 固定传 0（对齐 Vue signApi 直接发 query.value，待审核单据 signflag=0）
    params['signflag'] = 0;
    params['reviewremark'] = _formData['reviewremark']?.toString() ?? '';
    // 撤回状态：显式设置 reviewsignflag=2，避免 _loadData 异步刷新 _formData 导致丢失
    if (isWithdraw) {
      params['reviewsignflag'] = 2;
    } else {
      // reviewsignflag: 0=驳回保持不变，其余设为1（通过）
    ...
    setState(() => _saving = true);
    request(HttpApi.financeSupplierPaySign, params).then((_) {
      if (mounted) {
        Toast.show('审核成功');
        _loadData();
      }
  ...
  Future<void> _handleZf() async {
    if (!PermissionUtils.checkPermission('014406', showTip: false)) {
      Toast.show('你无权作废供应商结算，请在后台修改权限');
      return;
    }
    final confirm = await showDialog<bool>(
      context: context,
    ...
    setState(() => _saving = true);
    try {
      await request(HttpApi.financeSupplierPayZf, _formData);
      if (mounted) {
        Toast.show('作废成功');
  ...
  Future<void> _handleDelete() async {
    if (!PermissionUtils.checkPermission('014404', showTip: false)) {
      Toast.show('你无权删除供应商结算，请在后台修改权限');
      return;
    }
    final confirm = await showDialog<bool>(
      context: context,
    ...
    try {
      ...
      final billid =
          widget.billid.isNotEmpty ? widget.billid : (_formData['billid']?.toString() ?? '');
      await request(HttpApi.financeSupplierPayDelete, {'billid': billid});
      if (mounted) {
        Toast.show('删除成功');
  ...
  Future<void> _handleRetsign() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('提示'),
        content: const Text('撤回单据后，所有审批步骤需重新处理！是否撤回该供应商结算单？'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('取消')),
          TextButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('确定')),
        ],
    ...
    if (confirm != true) return;
    // 撤回：设置 reviewsignflag=2 后执行保存
    _formData['reviewsignflag'] = 2;
    await _handleSave(1);
  }

  void _handlePrint() {
    request(HttpApi.cgorderPrint, {'menuid': '100601', 'data': _formData}).then((_) {
  ...
  /// 居中加载图标卡片（透明遮罩，内容保持可见，避免保存/审核时白屏）
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        centerTitle: true,
        title: Text(_isEdit ? '供应商结算详情' : '新增供应商结算',
      ...
      body: Stack(children: [
        SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 8),
            _buildFormCard([
              _buildSelectRow('供应商', _supname,
                  onTap: !_isReadonly && !_isEdit ? _selectSupplier : null, required: true),
              _buildSelectRow('机构', _bstorename,
                  onTap: !_isReadonly && !_isEdit ? _selectStore : null, required: true),
              _buildSelectRow('选择单据', _suppayflow.isNotEmpty ? '单据数量：$_billnum，总金额：$_billamt' : '',
            ...
            Container(
              ...
              child: Column(children: [
                ...
                _buildSelectRow('付款账户', _bankname,
  ...
  /// 结算信息行：左标题 + 右输入框（FocusNode 失焦触发计算，对齐 Vue @blur）
  ...
  /// 结算信息行：左标题 + 右只读数值
  static Widget _buildSettleReadRow(String label, String value) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 14),
      decoration: const BoxDecoration(border: Border(bottom: BorderSide(color: Color(0xFFF0F0F0)))),
      child: Row(children: [
  ...
  Widget _buildVoidedButtons() {
    return _wrapBottomBar([_primaryBtn('打印', _handlePrint)]);
  }
  ...
  Widget _buildBillViewSheet(BuildContext ctx) {
    return SafeArea(
      ...
      decoration: const BoxDecoration(
          color: Color(0xFFF5F6FA), borderRadius: BorderRadius.vertical(top: Radius.circular(16))),
      child: Column(children: [
        ...
        Expanded(
          ...
          itemCount: _suppayflow.length,
          itemBuilder: (_, i) {
            final bill = _suppayflow[i];
            final typeName = _SelectBillSheetState._billTypeName(bill['billtype']);
            final createtime =
                bill['createtime']?.toString() ?? bill['supbilldate']?.toString() ?? '';
            return RepaintBoundary(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Flexible(
                      child: Text(bill['supbillno']?.toString() ?? '',
                          maxLines: 1,
...
// =================== 选择单据弹窗（对齐 Vue selectBill.vue）==================
class _SelectBillSheet extends StatefulWidget {
  const _SelectBillSheet({
    required this.bills,
...
class _SelectBillSheetState extends State<_SelectBillSheet> {
  ...
  void initState() {
    ...
    _all = widget.bills
        .map((e) => Map<String, dynamic>.from(e)
          ..['_selected'] = _selectedIds.contains(e['supbillid']?.toString() ?? ''))
        .toList();
    ...
    _applyFilter();
    // 外层 getSupplierBill 请求仍在进行中：弹窗内自行重新拉取并显示 loading
  ...
  void _applyFilter() {
    ...
    final list = _all.where((e) {
      ...
      if (kw.isNotEmpty &&
          !(e['supbillno']?.toString() ?? '').contains(kw) &&
  ...
  void _toggle(Map<String, dynamic> bill) {
    final id = bill['supbillid']?.toString() ?? '';
    setState(() {
    ...
    try {
      ...
      if (data is List) {
        ...
        for (final b in bills) {
          _selectedIds.add(b['supbillid']?.toString() ?? '');
          b['_selected'] = true;
        }
        _all = bills;
        _applyFilter();
  ...
  static void _recalcBill(Map<String, dynamic> item) {
    final nowamt = double.tryParse(item['nowamt']?.toString() ?? '') ?? 0;
    final result = _SupplierPayEditPageState._calculatePayment(nowamt, {
      'nowamt': nowamt,
      'freeamt': 0,
  ...
  Widget _buildBillCard(Map<String, dynamic> bill) {
    ...
    final createtime = bill['createtime']?.toString() ?? bill['supbilldate']?.toString() ?? '';
    return RepaintBoundary(
        child: GestureDetector(
      onTap: () => _toggle(bill),
      child: Container(
