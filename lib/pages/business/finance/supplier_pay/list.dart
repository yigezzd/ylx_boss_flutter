...
/// 供应商结算列表页 —— 对齐 boss 项目 purchasePlierpay.vue
class SupplierPayListPage extends StatefulWidget {
  const SupplierPayListPage({super.key});

  @override
  State<SupplierPayListPage> createState() => _SupplierPayListPageState();
}

class _SupplierPayListPageState extends State<SupplierPayListPage> with TickerProviderStateMixin {
  ...
  String _filterSupsignflag = ''; // 供应商对账状态
  ...
  void initState() {
    ...
    if (!PermissionUtils.checkPermission('014401', showTip: false)) {
      Toast.show('你无权查看供应商结算，请在后台修改权限');
      return;
    }
    _loadReviewConfig();
  }
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('供应商结算单');
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('供应商结算单', force: true);
    if (!mounted) return;
    _applyTabCount(show ? 4 : 3);
  }
  ...
  Future<void> _selectStore() async {
    // 对齐 Vue purchasePlierpay.vue：登录门店 supflag==1 时传 supflag: 1，且不传 storetypes
  ...
  Future<void> _loadData() {
    ...
    return request(HttpApi.financeSupplierPayGetList, {
      'is_page': 1,
      'field': 'createtime',
      'type': 'desc',
      'cond': '',
      ...
      if (_filterSupsignflag.isNotEmpty) 'supsignflag': _filterSupsignflag,
    }).then((result) {
  ...
  void _openFilterSheet() {
    ...
    String tmpSupsignflag = _filterSupsignflag;
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _FilterSelectRow(
                              label: '供应商',
                              value: tmpSupname.isEmpty ? '全部供应商' : tmpSupname,
                              onTap: () =>
                                  _openSupSelector(ctx, setSheetState, initialSelectedId: tmpSupid,
                          ...
                          _FilterSelectRow(
                              label: '付款账户',
                              value: tmpBankname.isEmpty ? '全部付款账户' : tmpBankname,
                          ...
                          Wrap(
                            ...
                            runSpacing: 6,
                            children: [
                              _FilterTag('全部', '', tmpSupsignflag,
                                  (v) => setSheetState(() => tmpSupsignflag = v)),
                              _FilterTag('待确定', '0', tmpSupsignflag,
                                  (v) => setSheetState(() => tmpSupsignflag = v)),
                              _FilterTag('同意', '1', tmpSupsignflag,
                                  (v) => setSheetState(() => tmpSupsignflag = v)),
                              _FilterTag('不同意', '2', tmpSupsignflag,
                                  (v) => setSheetState(() => tmpSupsignflag = v)),
                            ],
                          ),
                        ],
                          ...
                          child: ElevatedButton(
                            onPressed: () {
                              ...
                              setState(() {
                                ...
                                _filterSupsignflag = tmpSupsignflag;
                                _page = 1;
                                _list = [];
                                _hasMore = true;
                              });
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return RepaintBoundary(
                          child: _SupplierPayCard(
                            item: _list[index],
                            onTap: () {
                              if (!PermissionUtils.checkPermission('014408', showTip: false)) {
                              ...
                              NavigatorUtils.pushResult(
                                context,
                                '${FinanceRouter.supplierPayEdit}?billid=$billid',
                                // 对齐采购入库：返回后重评审批配置（已驳回 tab 及时出现/消失）
                                (_) => _onRefresh(),
                              );
                            },
  ...
  // ── 选择器 ──
  Future<void> _openSupSelector(
    BuildContext parentCtx,
    StateSetter setSheetState, {
    required void Function(String id, String name) onSelected,
    String? initialSelectedId,
  }) async {
    final result = await SelectSupplierPage.show(parentCtx, initialSelectedId: initialSelectedId);
    if (result != null) {
      onSelected(
...
// 卡片组件
// ─────────────────────────────────────────────────
class _SupplierPayCard extends StatelessWidget {
  const _SupplierPayCard({required this.item, required this.onTap});
...
class _FilterTag extends StatelessWidget {
  const _FilterTag(this.label, this.value, this.current, this.onTap);
  final String label;
  final String value;
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Text(label,
            style: TextStyle(fontSize: 12, color: active ? Colors.white : const Color(0xFF333333))),
      ),
    );
  }
}
