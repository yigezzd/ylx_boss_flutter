...
/// 费用收支单列表页 —— 对齐 boss 项目 costRevenueExpenses.vue
class CostRevenueListPage extends StatefulWidget {
  const CostRevenueListPage({super.key});

  @override
  State<CostRevenueListPage> createState() => _CostRevenueListPageState();
}

class _CostRevenueListPageState extends State<CostRevenueListPage> with TickerProviderStateMixin {
  ...
  // ── 筛选参数 ──
  String _filterBilltype = '';
  String _filterBilltypename = '';
  String _filterFeeitem = '';
  String _filterFeename = '';
  String _filterCreateid = '';
  String _filterCreatename = '';
  String _filterHandlerid = '';
  String _filterHandlername = '';
  String _filterPaytype = '';
  String _filterPaytypename = '';
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('费用收支单');
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('费用收支单', force: true);
    if (!mounted) return;
    _applyTabCount(show ? 4 : 3);
  }
  ...
  Future<void> _loadData() {
    ...
    return request(HttpApi.financeCostRevenueGetList, {
      'datetype': '1',
      'is_page': 1,
      'field': 'createtime',
      'type': 'desc',
      ...
      'sids': _sids,
      if (_filterBilltype.isNotEmpty) 'billtype': _filterBilltype,
      if (_filterFeeitem.isNotEmpty) 'feeitem': _filterFeeitem,
      if (_filterCreateid.isNotEmpty) 'createid': _filterCreateid,
      if (_filterHandlerid.isNotEmpty) 'handlerid': _filterHandlerid,
      if (_filterPaytype.isNotEmpty) 'paytype': _filterPaytype,
    }).then((result) {
  ...
  void _openFilterSheet() {
    ...
    String tmpBilltype = _filterBilltype;
    String tmpBilltypename = _filterBilltypename;
    String tmpFeeitem = _filterFeeitem;
    String tmpFeename = _filterFeename;
    ...
    String tmpPaytype = _filterPaytype;
    String tmpPaytypename = _filterPaytypename;
    int tmpActiveQuickTimeId = _activeQuickTimeId ?? 4;
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _FilterSelectRow(
                              label: '业务类型',
                              value: tmpBilltypename.isEmpty ? '全部业务类型' : tmpBilltypename,
                              onTap: () => _openBilltypeSelector(ctx, setSheetState,
                                      activeId: tmpBilltype, onSelected: (String id, String name) {
                                    setSheetState(() {
                                      tmpBilltype = id;
                                      tmpBilltypename = name;
                                    });
                                  })),
                          _FilterSelectRow(
                              label: '收支项目',
                              value: tmpFeename.isEmpty ? '全部收支项目' : tmpFeename,
                              onTap: () => _openFeeitemSelector(ctx, setSheetState,
                                      initialSelectedId: tmpFeeitem,
                          ...
                          _FilterSelectRow(
                              ...
                              onTap: () => _openUserSelector(ctx, setSheetState,
                                      ...
                                      title: '选择制单人', onSelected: (String id, String name) {
                                    setSheetState(() {
                                      tmpCreateid = id;
                                      tmpCreatename = name;
                                    });
                          ...
                          _FilterSelectRow(
                              ...
                              onTap: () => _openUserSelector(ctx, setSheetState,
                                      ...
                                      title: '选择经手人', onSelected: (String id, String name) {
                                    setSheetState(() {
                                      tmpHandlerid = id;
                                      tmpHandlername = name;
                                    });
                          ...
                          _FilterSelectRow(
                              label: '收支方式',
                              value: tmpPaytypename.isEmpty ? '全部收支方式' : tmpPaytypename,
                              onTap: () => _openPaytypeSelector(ctx, setSheetState,
                                      initialSelectedId: tmpPaytype,
                                      onSelected: (String id, String name) {
                                    setSheetState(() {
                                      tmpPaytype = id;
                                      tmpPaytypename = name;
                          ...
                          child: ElevatedButton(
                            onPressed: () {
                              ...
                              setState(() {
                                ...
                                _filterBilltype = tmpBilltype;
                                _filterBilltypename = tmpBilltypename;
                                _filterFeeitem = tmpFeeitem;
                                _filterFeename = tmpFeename;
                                _filterCreateid = tmpCreateid;
                                _filterCreatename = tmpCreatename;
                                _filterHandlerid = tmpHandlerid;
                                _filterHandlername = tmpHandlername;
                                _filterPaytype = tmpPaytype;
                                _filterPaytypename = tmpPaytypename;
                                _page = 1;
                                _list = [];
                                _hasMore = true;
                              });
  ...
  // ── 子选择器 ──
  Future<void> _openBilltypeSelector(
    ...
    final result = await CommonSelectSheet.show(
      ...
      fetchData: (search, page) async {
        return <String, dynamic>{'list': list, 'has_more': false};
      },
      nameField: 'label',
      codeField: '',
    ...
    if (result != null) {
      onSelected(
        result['id']?.toString() ?? '',
        result['label'] == '全部' ? '' : (result['label']?.toString() ?? ''),
      );
    }
  }

  Future<void> _openFeeitemSelector(
    BuildContext parentCtx,
    StateSetter setSheetState, {
    required void Function(String id, String name) onSelected,
  ...
  Future<void> _openPaytypeSelector(
    BuildContext parentCtx,
    StateSetter setSheetState, {
    required void Function(String id, String name) onSelected,
    String? initialSelectedId,
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return RepaintBoundary(
                          child: _CostRevenueCard(
                            item: _list[index],
                            onTap: () {
                              if (!PermissionUtils.checkPermission('014508', showTip: false)) {
                              ...
                              NavigatorUtils.pushResult(
                                context,
                                '${FinanceRouter.costRevenueEdit}?billid=$billid',
                                // 对齐采购入库：返回后重评审批配置（已驳回 tab 及时出现/消失）
                                (_) => _onRefresh(),
                              );
                            },
...
// 卡片组件
// ─────────────────────────────────────────────────
class _CostRevenueCard extends StatelessWidget {
  const _CostRevenueCard({required this.item, required this.onTap});
  final Map<String, dynamic> item;
  final VoidCallback onTap;

  String _billtypeLabel(String? type) {
    const map = {'1': '收入单', '2': '支出单', '3': '转账单'};
    return map[type] ?? '';
  }
  ...
  Widget build(BuildContext context) {
    ...
    final payamt = item['payamt']?.toString() ?? '0';
    final formattedPayamt = MathUtils.formatDecimal(3, double.tryParse(payamt) ?? 0);
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      '业务类型：${_billtypeLabel(item['billtype']?.toString())}',
                      style: const TextStyle(fontSize: 12, color: Color(0xFF7A7A7A)),
                      overflow: TextOverflow.ellipsis,
                    ),
                  ),
          ...
          children: [
            ...
            Expanded(
              child: Text(
                ...
                style: TextStyle(
                  fontSize: 14,
                  color: value.startsWith('全部') ? const Color(0xFF9CA3AF) : const Color(0xFF374151),
                ),
              ),
            ),
