...
/// 新品申请 - 列表页
/// 参考 Vue boss 项目 newApplication/index.vue
class NewApplicationListPage extends StatefulWidget {
  const NewApplicationListPage({super.key});

  @override
  State<NewApplicationListPage> createState() => _NewApplicationListPageState();
}

class _NewApplicationListPageState extends State<NewApplicationListPage>
  ...
  void initState() {
    ...
    _endTime = _formatDate(now);
    _startTime = _formatDate(now.subtract(const Duration(days: 29)));

    // 查看权限校验
    if (!PermissionUtils.checkPermission('010302', showTip: false)) {
      Toast.show('你无权查看新品申请，请在后台修改权限');
    } else {
      _loadData();
    }
  }
  ...
  void _onTabChanged() {
    ...
    _status = _tabs[_tabController.index]['key']!;
    _page = 1;
    _list = [];
    _hasMore = true;
    _loadData();
  ...
  Future<void> _loadData() async {
    ...
    final params = <String, dynamic>{
      ...
      if (_startTime.isNotEmpty) 'starttime': '$_startTime 00:00:00',
      if (_endTime.isNotEmpty) 'endtime': '$_endTime 23:59:59',
    };

    try {
      final result = await request(HttpApi.applyNewProGetList, params);
  ...
  Future<void> _add() async {
    if (!PermissionUtils.checkPermission('010301', showTip: false)) {
      Toast.show('你无权新增新品申请，请在后台修改权限');
      return;
    }
    final refresh = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => const CommodityAddPage(applynewflag: 1),
      ),
    );
  ...
  Future<void> _edit(Map<String, dynamic> item) async {
    if (!PermissionUtils.checkPermission('010305', showTip: false)) {
      Toast.show('你无权编辑新品申请，请在后台修改权限');
      return;
    }
    final refresh = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => CommodityAddPage(
            productData: item,
            applynewflag: item['status'] == 3 ? 0 : 1,
            status: item['status'] is int
                ? item['status'] as int
                : int.tryParse(item['status']?.toString() ?? '')),
      ),
    );
    if ((refresh ?? false) && mounted) {
      _onRefresh();
  ...
  void _openFilterSheet() {
    String tmpStart = _startTime;
    String tmpEnd = _endTime;
                                ...
                                child: GestureDetector(
                                  onTap: () async {
                                    final picked = await showDatePicker(
                                      context: ctx,
                                      initialDate: DateTime.tryParse(tmpStart) ?? DateTime.now(),
                              ...
                              Expanded(
                                child: GestureDetector(
                                  onTap: () async {
                                    final picked = await showDatePicker(
                                      context: ctx,
                                      initialDate: DateTime.tryParse(tmpEnd) ?? DateTime.now(),
                                      firstDate: DateTime(2020),
                                      lastDate: DateTime(2030),
                                    );
                                    if (picked != null) {
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return RepaintBoundary(
                    child: _NewApplicationItemCard(
                      item: _list[index],
                      onTap: () => _edit(_list[index]),
                    ),
                  );
                },
              ),
...
/// 新品申请列表项卡片
class _NewApplicationItemCard extends StatelessWidget {
  const _NewApplicationItemCard({required this.item, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    final String rejectionreason = item['rejectionreason']?.toString() ?? '';
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // 驳回原因（仅已驳回时显示）
            if (status == '4' && rejectionreason.isNotEmpty)
              Padding(
