...
class InventoryQuickcheckListPage extends StatefulWidget {
  const InventoryQuickcheckListPage({super.key});

  @override
  State<InventoryQuickcheckListPage> createState() => _InventoryQuickcheckListPageState();
}

class _InventoryQuickcheckListPageState extends State<InventoryQuickcheckListPage>
    with SingleTickerProviderStateMixin, LogPageMixin<InventoryQuickcheckListPage> {
  @override
  String get logPageName => '快速盘点列表';
  ...
  void initState() {
    ...
    _loadStoreName();
    logEnter();
    if (!PermissionUtils.checkPermission('013801', showTip: false)) {
      Toast.show('你无权查看快速盘点，请在后台修改权限');
      return;
    }
    _loadData();
  }
  ...
  Future<void> _loadData() {
    ...
    return request('/stockcheckfast/findList', {
      'is_page': 1,
      'cond': '',
      'field': 'createtime',
      'type': 'desc',
                ...
                Expanded(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    _buildFilterRow(
                        ...
                        onTap: () async {
                          final r = await SelectBuyerPage.show(ctx, initialSelectedId: tmpCreateid);
                          if (r != null)
                            setSheetState(() {
            ...
            child: ColoredBox(
                ...
                child: Column(children: [
                        ...
                        GestureDetector(
                            onTap: () async {
                              ...
                              await Navigator.push<bool>(
                                  context,
                                  MaterialPageRoute(
                                      builder: (_) => const InventoryQuickcheckAddPage()));
                              _onRefresh();
                            },
                            child: Container(
                                width: 36,
                  ...
                  TabBar(
                      ...
                      tabs: const [Tab(text: '全部'), Tab(text: '待审核'), Tab(text: '已审核'), Tab(text: '已驳回')]),
                ]))),
      ),
                        ...
                        child: _QuickcheckCard(
                            ...
                            onTap: () async {
                              ...
                              await Navigator.push<bool>(
                                  ...
                                  MaterialPageRoute(
                                      builder: (_) =>
                                          InventoryQuickcheckAddPage(billData: _list[index])));
                              _onRefresh();
                            }));
                  })),
    );
...
class _QuickcheckCard extends StatelessWidget {
  const _QuickcheckCard({required this.item, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    final bool isSigned = signflag == '1';
    final bool isRejected = signflag == '2';
                ...
                Row(children: [
                  Expanded(
                      child: Text('盘点机构：$storename',
                          ...
                          overflow: TextOverflow.ellipsis)),
                  Text(
                      isSigned ? '已审核' : (isRejected ? '已驳回' : '待审核'),
                      style: TextStyle(
                          fontSize: 14,
                          fontWeight: FontWeight.w500,
                          color: isSigned
                              ? const Color(0xFF00A870)
                              : (isRejected ? const Color(0xFFFF9900) : const Color(0xFFD54B5A)))),
                ]),
                const SizedBox(height: 8),
                Text('盘点单号：$billno',
