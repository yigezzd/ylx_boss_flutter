...
class InventoryCostchangeListPage extends StatefulWidget {
  const InventoryCostchangeListPage({super.key});

  @override
  State<InventoryCostchangeListPage> createState() => _InventoryCostchangeListPageState();
}

class _InventoryCostchangeListPageState extends State<InventoryCostchangeListPage>
    with SingleTickerProviderStateMixin, LogPageMixin<InventoryCostchangeListPage> {
  @override
  String get logPageName => '成本变更单列表';
  ...
  void initState() {
    ...
    _loadStoreName();
    logEnter();
    if (!PermissionUtils.checkPermission('014201', showTip: false)) {
      Toast.show('你无权查看成本变更，请在后台修改权限');
      return;
    }
    _loadData();
  }
  ...
  Future<void> _loadData() {
    ...
    return request(HttpApi.costChangeFindList, {
      'is_page': 1,
      'cond': '',
      'field': 'createtime',
      'type': 'desc',
                        ...
                        GestureDetector(
                            onTap: () async {
                              ...
                              await Navigator.push<bool>(
                                  context,
                                  MaterialPageRoute(
                                      builder: (_) => const InventoryCostchangeAddPage()));
                              _onRefresh();
                            },
                            child: Container(
                                width: 36,
                        ...
                        child: _CostchangeCard(
                            ...
                            onTap: () async {
                              ...
                              await Navigator.push<bool>(
                                  ...
                                  MaterialPageRoute(
                                      builder: (_) =>
                                          InventoryCostchangeAddPage(billData: _list[index])));
                              _onRefresh();
                            }));
                  })),
    );
...
class _CostchangeCard extends StatelessWidget {
  const _CostchangeCard({required this.item, required this.onTap});
