...
class InventoryPlanListPage extends StatefulWidget {
  const InventoryPlanListPage({super.key});

  @override
  State<InventoryPlanListPage> createState() => _InventoryPlanListPageState();
}

class _InventoryPlanListPageState extends State<InventoryPlanListPage>
    with SingleTickerProviderStateMixin, LogPageMixin<InventoryPlanListPage> {
  @override
  String get logPageName => '盘点计划列表';

  late TabController _tabController;

  int _statusIndex = 0; // 0=全部 1=待盘点 2=盘点中 3=已盘点
  int _page = 1;
  bool _loading = false;
  bool _hasMore = true;
  ...
  void initState() {
    ...
    _loadStoreName();
    logEnter();
    if (!PermissionUtils.checkPermission('013601', showTip: false)) {
      Toast.show('你无权查看盘点计划，请在后台修改权限');
      return;
    }
    _loadData();
  }
  ...
  /// signflag 映射：0=全部 1=待盘点(0) 2=盘点中(2) 3=已盘点(1)
  ...
  Future<void> _loadData() {
    ...
    return request('/stockplan/findList', {
      'is_page': 1,
      'cond': '',
      'field': 'createtime',
      'type': 'desc',
      ...
      if (_filterChecktype.isNotEmpty) 'checktype': _filterChecktype,
    }).then((result) {
  ...
  void _openFilterSheet() {
    ...
    String tmpChecktype = _filterChecktype;
                      ...
                      child: Column(
                        ...
                        children: [
                            ...
                            Expanded(
                              child: _buildDateChip(
                                ...
                                onTap: () async {
                                  ...
                                  final picked = await showCommonDatePicker(ctx, initial: tmpStart);
                                  if (picked != null) {
                                    if (picked
                                        .isAfter(DateTime(tmpEnd.year, tmpEnd.month, tmpEnd.day))) {
                          ...
                          Wrap(
                            ...
                            children: [
                              const _FilterTag('全部', ''),
                              const _FilterTag('全场盘点', '1'),
                              const _FilterTag('分类盘点', '2'),
                              const _FilterTag('供应商盘点', '3'),
                              const _FilterTag('品牌盘点', '4'),
                            ].map((tag) {
                              final active = tmpChecktype == tag.value;
                              return GestureDetector(
                                onTap: () => setSheetState(() => tmpChecktype = tag.value),
                                child: Container(
                  ...
                  GestureDetector(
                    onTap: () async {
                      ...
                      await Navigator.push<bool>(
                        context,
                        MaterialPageRoute(
                          builder: (_) => const InventoryPlanAddPage(),
                        ),
                      );
                      _onRefresh();
                    },
                    ...
                    child: _PlanCard(
                      ...
                      onTap: () async {
                        ...
                        await Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => InventoryPlanAddPage(billData: _list[index]),
                          ),
                        );
                        _onRefresh();
                      },
...
class _PlanCard extends StatelessWidget {
  ...
  static Color _statusColor(String? signflag) {
    if (signflag == '1') return const Color(0xFF00A870); // 已盘点
    if (signflag == '2') return const Color(0xFF09B8EE); // 盘点中
    return const Color(0xFFD54B5A); // 待盘点
  }

  static String _statusLabel(String? signflag) {
    if (signflag == '1') return '已盘点';
    if (signflag == '2') return '盘点中';
    return '待盘点';
  }

  @override
  Widget build(BuildContext context) {
    final String planname = item['planname']?.toString() ?? '-';
