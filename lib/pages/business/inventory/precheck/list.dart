...
class InventoryPrecheckListPage extends StatefulWidget {
  const InventoryPrecheckListPage({super.key});

  @override
  State<InventoryPrecheckListPage> createState() => _InventoryPrecheckListPageState();
}

class _InventoryPrecheckListPageState extends State<InventoryPrecheckListPage>
    with SingleTickerProviderStateMixin, LogPageMixin<InventoryPrecheckListPage> {
  @override
  String get logPageName => '预盘单列表';
  ...
  void initState() {
    ...
    _loadStoreName();
    logEnter();
    if (!PermissionUtils.checkPermission('013701', showTip: false)) {
      Toast.show('你无权查看预盘单，请在后台修改权限');
      return;
    }
    _loadData();
  }
  ...
  Future<void> _loadData() {
    ...
    return request('/stockprecheck/findList', {
      'is_page': 1,
      'cond': '',
      'field': 'createtime',
      'type': 'desc',
                            ...
                            return GestureDetector(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  border: Border.all(
                                    color:
                                        active ? const Color(0xFF006EFF) : const Color(0xFF999999),
                                  ),
                                  borderRadius: BorderRadius.circular(4),
                                ),
                                child: Text(tag.label,
                    ...
                    Expanded(
                      child: GestureDetector(
                        ...
                        child: Container(
                          ...
                          alignment: Alignment.center,
                          child:
                              const Text('确定', style: TextStyle(fontSize: 15, color: Colors.white)),
                        ),
                      ),
                    ),
                  ]),
                  ...
                  GestureDetector(
                    onTap: () async {
                      ...
                      await Navigator.push<bool>(
                        context,
                        MaterialPageRoute(
                          builder: (_) => const InventoryPrecheckAddPage(),
                        ),
                      );
                      _onRefresh();
                    },
                    ...
                    child: _PrecheckCard(
                      ...
                      onTap: () async {
                        ...
                        await Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => InventoryPrecheckAddPage(billData: _list[index]),
                          ),
                        );
                        _onRefresh();
                      },
...
class _PrecheckCard extends StatelessWidget {
  const _PrecheckCard({required this.item, required this.onTap});
