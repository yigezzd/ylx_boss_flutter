...
class InventoryOtherinListPage extends StatefulWidget {
  const InventoryOtherinListPage({super.key});

  @override
  State<InventoryOtherinListPage> createState() => _InventoryOtherinListPageState();
}

class _InventoryOtherinListPageState extends State<InventoryOtherinListPage>
    with SingleTickerProviderStateMixin, LogPageMixin<InventoryOtherinListPage> {
  @override
  String get logPageName => '其他入库单列表';
  ...
  void initState() {
    ...
    _loadStoreName();
    logEnter();
    if (!PermissionUtils.checkPermission('014001', showTip: false)) {
      Toast.show('你无权查看其他入库单，请在后台修改权限');
      return;
    }
    _loadData();
  }
                        ...
                        GestureDetector(
                            onTap: () async {
                              ...
                              await Navigator.push<bool>(
                                  context,
                                  MaterialPageRoute(
                                      builder: (_) => const InventoryOtherinAddPage()));
                              _onRefresh();
                            },
                            child: Container(
                                width: 36,
                        ...
                        child: _OtherinCard(
                            ...
                            onTap: () async {
                              ...
                              await Navigator.push<bool>(
                                  ...
                                  MaterialPageRoute(
                                      builder: (_) =>
                                          InventoryOtherinAddPage(billData: _list[index])));
                              _onRefresh();
                            }));
                  })),
    );
...
class _OtherinCard extends StatelessWidget {
  const _OtherinCard({required this.item, required this.onTap});
