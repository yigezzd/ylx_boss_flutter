...
class InventoryOtheroutListPage extends StatefulWidget {
  const InventoryOtheroutListPage({super.key});

  @override
  State<InventoryOtheroutListPage> createState() => _InventoryOtheroutListPageState();
}

class _InventoryOtheroutListPageState extends State<InventoryOtheroutListPage>
    with SingleTickerProviderStateMixin, LogPageMixin<InventoryOtheroutListPage> {
  @override
  String get logPageName => '其他出库单列表';
  ...
  void initState() {
    ...
    _loadStoreName();
    logEnter();
    if (!PermissionUtils.checkPermission('013901', showTip: false)) {
      Toast.show('你无权查看其他出库单，请在后台修改权限');
      return;
    }
    _loadData();
  }
  ...
  /// 单据类型筛选选项（对齐 lxAss createList）
  static const List<Map<String, String>> _billtypeOptions = [
    {'id': '', 'name': '全部'},
    {'id': '1', 'name': '报损'},
    {'id': '2', 'name': '领用'},
    {'id': '4', 'name': '其他出库'},
  ...
  /// 对齐 lxAss getList：全部→billtypes=[1,2,4]不传outtype；选其他出库(4)→billtypes=[4]+outtype=1
  Map<String, dynamic> get _billtypeParams {
    if (_filterBilltype.isEmpty) {
      return {
        'billtypes': [1, 2, 4]
      };
    }
    final params = <String, dynamic>{
      'billtypes': [int.tryParse(_filterBilltype) ?? 4]
    };
    if (_filterBilltype == '4') params['outtype'] = 1;
    return params;
  }
  ...
  Future<void> _loadData() {
    ...
    return request(HttpApi.stockModifyFindList, {
      'is_page': 1,
      'cond': '',
      'field': 'createtime',
      'type': 'desc',
              ...
              child: Column(children: [
                ...
                Expanded(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    _buildFilterRow(
                        label: '单据类型',
                        value: tmpBilltype.isEmpty ? '全部类型' : tmpBilltypename,
                        onTap: () async {
                          final picked =
                              await _showOptionSheet(ctx, '选择单据类型', _billtypeOptions, tmpBilltype);
                          if (picked != null)
                            setSheetState(() {
                              tmpBilltype = picked['id'] ?? '';
                              tmpBilltypename = picked['name'] ?? '';
                            });
                        }),
                    _buildFilterRow(
                        label: '仓库',
                        ...
                        onTap: () async {
                          final r = await SelectWarehousePage.show(ctx,
                              initialSelectedId: tmpCounterid, showAll: true);
                          if (r != null)
                            setSheetState(() {
                              tmpCounterid = r['counterid']?.toString() ?? '';
                              tmpCountername = r['countername']?.toString() ?? '';
                            });
                        }),
                    _buildFilterRow(
                        label: '制单人',
  ...
  /// 通用单选底部抽屉（用于单据类型等简单选项）
  static Future<Map<String, String>?> _showOptionSheet(
      BuildContext ctx, String title, List<Map<String, String>> options, String selectedId) {
        ...
        return Container(
          ...
          child: SafeArea(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const Divider(height: 1, color: Color(0xFFE5E7EB)),
              ...options.map((opt) {
                final active = opt['id'] == selectedId;
                return GestureDetector(
                  behavior: HitTestBehavior.opaque,
                      ...
                      Expanded(
                          child: Text(opt['name'] ?? '',
                              style: TextStyle(
                                  fontSize: 14,
                                  color:
                                      active ? const Color(0xFF006EFF) : const Color(0xFF333333)))),
                        ...
                        GestureDetector(
                            onTap: () async {
                              ...
                              await Navigator.push<bool>(
                                  context,
                                  MaterialPageRoute(
                                      builder: (_) => const InventoryOtheroutAddPage()));
                              _onRefresh();
                            },
                            child: Container(
                                width: 36,
                        ...
                        child: _OtheroutCard(
                            ...
                            onTap: () async {
                              ...
                              await Navigator.push<bool>(
                                  ...
                                  MaterialPageRoute(
                                      builder: (_) =>
                                          InventoryOtheroutAddPage(billData: _list[index])));
                              _onRefresh();
                            }));
                  })),
    );
...
class _OtheroutCard extends StatelessWidget {
  const _OtheroutCard({required this.item, required this.onTap});
