...
enum _OtheroutAction { none, save, sign, delete, print, withdraw }

class InventoryOtheroutAddPage extends StatefulWidget {
  const InventoryOtheroutAddPage({super.key, this.billData});
  final Map<String, dynamic>? billData;

  @override
  State<InventoryOtheroutAddPage> createState() => _InventoryOtheroutAddPageState();
}

class _InventoryOtheroutAddPageState extends State<InventoryOtheroutAddPage>
    with LogPageMixin<InventoryOtheroutAddPage> {
  @override
  String get logPageName => _isEdit ? '其他出库单详情' : '其他出库单新增';
  ...
  _OtheroutAction _submitAction = _OtheroutAction.none;
    ...
    request(HttpApi.stockModifyGetInfo, params).then((result) {
      ...
      if (data is Map<String, dynamic>) {
        setState(() {
          ...
          _billtype = int.tryParse(data['billtype']?.toString() ?? '') ?? 4;
  ...
  /// 计算明细行金额字段（对齐 lxAss amtJudge + defValSet）
  static void _computeAmts(Map<String, dynamic> item) {
    ...
    item['modifyqty'] = MathUtils.formatDecimal(1, MathUtils.subtract(stockqty, qty));
  }
  ...
  static void _defValSet(Map<String, dynamic> d) {
    ...
    d['costprice'] =
        // 价格类位数 formatDecimal(2)（对齐 lxAss 显示 formatDecimal(2, item.costprice)）
        MathUtils.formatDecimal(
            2,
            double.tryParse(d['emptycostprice']?.toString() ?? d['costprice']?.toString() ?? '0') ??
                0);
    ...
    d['stockqty'] =
        // 数量类位数 formatDecimal(1)
        MathUtils.formatDecimal(
            1,
            double.tryParse(
                    d['emptybatchstockqty']?.toString() ?? d['stockqty']?.toString() ?? '0') ??
                0);
  ...
  void _save({bool doSign = false}) {
    if (_isEdit) {
      if (!PermissionUtils.checkPermission('013903', showTip: false)) {
        Toast.show('你无权编辑其他出库单，请在后台修改权限');
        return;
      }
    } else {
      if (!PermissionUtils.checkPermission('013902', showTip: false)) {
        Toast.show('你无权新增其他出库单，请在后台修改权限');
        return;
      }
    }
    ...
    setState(() => _submitAction = _OtheroutAction.save);
    for (final item in _detailList) {
      _computeAmts(item);
      item['outflag'] = 1;
    }
    ...
    if (_recipientid != null && _recipientid!.isNotEmpty) {
      params['recipientid'] = _recipientid;
      params['recipientname'] = _recipientname;
    }
    ...
    request(HttpApi.stockModifySave, params).then((result) {
      ...
      if (mounted && !doSign) setState(() => _submitAction = _OtheroutAction.none);
    });
  }

  void _sign() {
    if (!PermissionUtils.checkPermission('013905', showTip: false)) {
      Toast.show('你无权审核其他出库单，请在后台修改权限');
      return;
    }
    ...
    if (_reviewFlowUsers.isNotEmpty) {
      _showApprovalDialog().then((approvalResult) {
        if (approvalResult != null && mounted) {
          ...
          setState(() => _submitAction = _OtheroutAction.none);
        }
      });
      return;
    }
    ...
    showDialog<bool>(
      ...
      if (confirm != true) {
        if (mounted) setState(() => _submitAction = _OtheroutAction.none);
        return;
      }
      _doSign();
    });
  ...
  void _doSign() {
    if (_billData == null) return;
    setState(() => _submitAction = _OtheroutAction.sign);
    ...
    if (_recipientid != null && _recipientid!.isNotEmpty) {
      signParams['recipientid'] = _recipientid;
      signParams['recipientname'] = _recipientname;
    }
    if (_handlerid != null && _handlerid!.isNotEmpty) {
  ...
  Future<void> _restsign() async {
    ...
    setState(() => _submitAction = _OtheroutAction.withdraw);
  ...
  Future<void> _delBill() async {
    if (_billData == null) return;
    if (!PermissionUtils.checkPermission('013904', showTip: false)) {
      Toast.show('你无权删除其他出库单，请在后台修改权限');
      return;
    }
    final confirm = await showDialog<bool>(
        context: context,
    ...
    if (confirm != true) return;
    setState(() => _submitAction = _OtheroutAction.delete);
  ...
  Future<void> _print() async {
    if (_billData == null) return;
    setState(() => _submitAction = _OtheroutAction.print);
    request(HttpApi.purchaseInstorePrint, {'menuid': '080201', 'data': _billData}).then((result) {
      if (!mounted) return;
      Toast.show('打印成功');
    }).whenComplete(() {
      if (mounted) setState(() => _submitAction = _OtheroutAction.none);
  ...
  Future<void> _selectRecipient() async {
    FocusScope.of(context).unfocus();
    final result = await SelectBuyerPage.show(context, initialSelectedId: _recipientid ?? '');
    if (result != null && mounted) {
      setState(() {
        _recipientid = result['buyerid']?.toString();
        _recipientname = result['buyername']?.toString();
      });
    }
  }

  // ── 选择单据类型 ──
  Future<void> _selectBilltype() async {
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildBilltypeOption(ctx, 1, '报损'),
            _buildBilltypeOption(ctx, 2, '领用'),
            _buildBilltypeOption(ctx, 4, '其他出库'),
  ...
  Widget _buildBilltypeOption(BuildContext ctx, int value, String label) {
    final active = _billtype == value;
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          const Spacer(),
          if (active) const Icon(Icons.check, size: 18, color: Color(0xFF006EFF)),
        ]),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
          ...
          title: Text(_isEdit ? '修改其他出库单' : '新增其他出库单',
                      ...
                      child: CustomScrollView(
                        ...
                        slivers: [
                              ...
                              child: Padding(
                                  ...
                                  child: Container(
                                      ...
                                      child: Column(children: [
                                        ...
                                        _buildSelectRow(
                                            label: '单据类型',
                                            value: _billtypeLabel,
                                            onTap: disabled ? null : _selectBilltype),
                                        _buildDivider(),
                                        if (_billtype == 2) ...[
                                          _buildSelectRow(
                                              label: '领用人',
                                              value: _recipientname ?? '',
                                              onTap: disabled ? null : _selectRecipient),
                                          _buildDivider(),
                                        ],
                                        _buildSelectRow(
                                            label: '经手人',
                          ...
                          SliverPersistentHeader(
                              pinned: true, delegate: _OtheroutStickyHeaderDelegate(state: this)),
                          if (_detailList.isEmpty)
                  ...
                  children: [
                    if (_detailList.isNotEmpty)
                      Padding(
                          ...
                          child: Text(
                              '出库数量：${MathUtils.formatDecimal(1, _totalQty)}，共 ${_detailList.length} 项',
                              style: const TextStyle(fontSize: 13, color: Color(0xFF6B7280)))),
                    Row(children: [
                      if (_isEdit && _isSigned) ...[
                        Expanded(
                            child: GestureDetector(
                                onTap: _submitAction != _OtheroutAction.none ? null : _print,
                                child: Container(
                                    height: 42,
                                    decoration: BoxDecoration(
                                    ...
                                    alignment: Alignment.center,
                                    child: _submitAction == _OtheroutAction.print
                                        ? const SizedBox(
                                            width: 20,
                                            height: 20,
                        ...
                        if (!_isWithdrawPending && _bolHandleTT) ...[
                          Expanded(
                              child: GestureDetector(
                                  onTap: _submitAction != _OtheroutAction.none ? null : _showMoreMenu,
                                  child: Container(
                                      height: 42,
                                      decoration: BoxDecoration(
                        ...
                        if (!_isRejected &&
                          ...
                          Expanded(
                              child: GestureDetector(
                                  onTap: _submitAction != _OtheroutAction.none ? null : () => _save(),
                                  child: Container(
                                      height: 42,
                                      decoration: BoxDecoration(
                                          color: const Color(0xFF006EFF),
                                          borderRadius: BorderRadius.circular(6)),
                                      alignment: Alignment.center,
                                      child: _submitAction == _OtheroutAction.save
                                          ? const SizedBox(
                                              width: 20,
                                              height: 20,
                                              child: CircularProgressIndicator(
                          ...
                          Expanded(
                              child: GestureDetector(
                                  onTap: _submitAction != _OtheroutAction.none
                                      ? null
                                      : () => _save(doSign: true),
                                  child: Container(
                                      height: 42,
                                      decoration: BoxDecoration(
                                          ...
                                          borderRadius: BorderRadius.circular(6)),
                                      alignment: Alignment.center,
                                      child: _submitAction == _OtheroutAction.sign
                                          ? const SizedBox(
                                              width: 20,
                                              height: 20,
                                              child: CircularProgressIndicator(
                        ...
                        if (_isRejected) ...[
                          ...
                          Expanded(
                              child: GestureDetector(
                                  onTap: _submitAction != _OtheroutAction.none ? null : _restsign,
                                  child: Container(
                                      height: 42,
                                      decoration: BoxDecoration(
                                          ...
                                          borderRadius: BorderRadius.circular(6)),
                                      alignment: Alignment.center,
                                      child: _submitAction == _OtheroutAction.withdraw
                                          ? const SizedBox(
                                              width: 20,
                                              height: 20,
                                              child: CircularProgressIndicator(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (disabled)
                          _buildDetailReadonlyField(
                              label: '出库数量', value: editQty.toStringAsFixed(1))
                        else
                ...
                Row(children: [
                  Expanded(
                      child: Text('现库存：${MathUtils.formatDecimal(1, stockqty)}',
                          style: const TextStyle(fontSize: 12, color: Color(0xFF7A7A7A)))),
                  Text('出库数量：${MathUtils.formatDecimal(1, qty)}',
                      textAlign: TextAlign.right,
                      style: const TextStyle(fontSize: 12, color: Color(0xFF7A7A7A))),
                ]),
              ])),
...
class _OtheroutStickyHeaderDelegate extends SliverPersistentHeaderDelegate {
  _OtheroutStickyHeaderDelegate({required this.state});
  final _InventoryOtheroutAddPageState state;
  @override
  double get minExtent => state._isSigned
      ? _InventoryOtheroutAddPageState._stickySignedExtent
      : _InventoryOtheroutAddPageState._stickyMinExtent;
  @override
  double get maxExtent => state._isSigned
      ? _InventoryOtheroutAddPageState._stickySignedExtent
      : _InventoryOtheroutAddPageState._stickyMaxExtent;
  @override
  ...
  @override
  bool shouldRebuild(covariant _OtheroutStickyHeaderDelegate oldDelegate) => true;
}
