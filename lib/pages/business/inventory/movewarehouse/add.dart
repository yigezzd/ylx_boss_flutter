...
enum _MwAction { none, save, sign, delete, print, withdraw }

class InventoryMovewarehouseAddPage extends StatefulWidget {
  const InventoryMovewarehouseAddPage({super.key, this.billData});
  final Map<String, dynamic>? billData;

  @override
  State<InventoryMovewarehouseAddPage> createState() => _InventoryMovewarehouseAddPageState();
}

class _InventoryMovewarehouseAddPageState extends State<InventoryMovewarehouseAddPage>
    with LogPageMixin<InventoryMovewarehouseAddPage> {
  @override
  String get logPageName => _isEdit ? '移仓单详情' : '移仓单新增';
  ...
  _MwAction _submitAction = _MwAction.none;
  ...
  void _loadDetail() {
    ...
    request(HttpApi.stockRollGetInfo, params).then((result) {
      if (!mounted) return;
  ...
  /// 转仓 amtJudge: amt=qty*outcostprice, inamt=qty*incostprice, sellamt=saleprice*qty
  static void _computeAmts(Map<String, dynamic> item) {
    final qty = double.tryParse(item['qty']?.toString() ?? '0') ?? 0;
    final outcostprice =
        double.tryParse(item['outcostprice']?.toString() ?? item['costprice']?.toString() ?? '0') ??
            0;
    final incostprice =
        double.tryParse(item['incostprice']?.toString() ?? item['costprice']?.toString() ?? '0') ??
            0;
    final saleprice =
        double.tryParse(item['saleprice']?.toString() ?? item['sellprice']?.toString() ?? '0') ?? 0;

    // amtJudge 对齐 lxAss: 金额 formatDecimal(3)
    item['amt'] = MathUtils.formatDecimal(3, MathUtils.mul(qty, outcostprice));
    item['inamt'] = MathUtils.formatDecimal(3, MathUtils.mul(qty, incostprice));
    item['sellamt'] = MathUtils.formatDecimal(3, MathUtils.mul(saleprice, qty));
  }
  ...
  static void _defValSet(Map<String, dynamic> d) {
    ...
    d['outcostprice'] = MathUtils.formatDecimal(
        2,
        double.tryParse(d['outemptycostprice']?.toString() ?? d['costprice']?.toString() ?? '0') ??
            0);
    d['incostprice'] = MathUtils.formatDecimal(
        2,
        double.tryParse(d['inemptycostprice']?.toString() ?? d['costprice']?.toString() ?? '0') ??
            0);
  ...
  static void _writeData(String key, Map<String, dynamic> d) {
    switch (key) {
      ...
      case 'outcostprice':
        d['outcostprice'] =
            MathUtils.formatDecimal(2, double.tryParse(d['outcostprice']?.toString() ?? '0') ?? 0);
        break;
      case 'incostprice':
        d['incostprice'] =
            MathUtils.formatDecimal(2, double.tryParse(d['incostprice']?.toString() ?? '0') ?? 0);
        break;
      case 'saleprice':
        d['saleprice'] =
  ...
  void _save({bool doSign = false}) {
    if (_isEdit) {
      if (!PermissionUtils.checkPermission('014103', showTip: false)) {
        Toast.show('你无权编辑移仓单，请在后台修改权限');
        return;
      }
    } else {
    ...
    if (_outcounterid == null || _outcounterid!.isEmpty) {
      Toast.show('请选择移出仓库');
      return;
    }
    if (_incounterid == null || _incounterid!.isEmpty) {
      Toast.show('请选择移入仓库');
      return;
    }
    if (_outcounterid == _incounterid) {
      Toast.show('移出仓库和移入仓库不能相同');
      return;
    }
    ...
    setState(() => _submitAction = _MwAction.save);
    for (final item in _detailList) {
      _computeAmts(item);
    }

    double billamt = 0, billinamt = 0, billqty = 0, billsellamt = 0;
    for (final item in _detailList) {
      billamt += double.tryParse(item['amt']?.toString() ?? '0') ?? 0;
      billinamt += double.tryParse(item['inamt']?.toString() ?? '0') ?? 0;
    ...
    final params = <String, dynamic>{
      ...
      'outcountername': _outcountername,
      'incounterid': _incounterid,
      'incountername': _incountername,
      'billtype': 3,
      'signflag': 0,
      'remark': _remarkCtrl.text.trim(),
      'fileLists': _fileLists,
      'billamt': MathUtils.formatDecimal(3, billamt),
      'billinamt': MathUtils.formatDecimal(3, billinamt),
    ...
    final isNew = !_isEdit;
    request(HttpApi.stockRollSave, params).then((result) {
      if (!mounted) return;
      final data = result['data'];
      ...
      if (mounted && !doSign) setState(() => _submitAction = _MwAction.none);
    });
  }

  void _sign() {
    if (!PermissionUtils.checkPermission('014105', showTip: false)) {
      Toast.show('你无权审核移仓单，请在后台修改权限');
      return;
    }
    ...
    if (_reviewFlowUsers.isNotEmpty) {
      _showApprovalDialog().then((approvalResult) {
        if (approvalResult != null && mounted) {
          ...
          setState(() => _submitAction = _MwAction.none);
        }
      });
      return;
    }
    ...
    showDialog<bool>(
      ...
      if (confirm != true) {
        if (mounted) setState(() => _submitAction = _MwAction.none);
        return;
      }
      _doSign();
    });
  ...
  void _doSign() {
    if (_billData == null) return;
    setState(() => _submitAction = _MwAction.sign);
    ...
    request(HttpApi.stockRollSign, signParams).then((result) {
      if (!mounted) return;
  ...
  Future<void> _restsign() async {
    ...
    setState(() => _submitAction = _MwAction.withdraw);
    final params = Map<String, dynamic>.from(_billData!);
    params['reviewsignflag'] = 2;
    params['reviewremark'] = '';
    params['signflag'] = 1;
    request(HttpApi.stockRollSign, params).then((result) {
      if (!mounted) return;
      Toast.show('撤回成功');
      _loadDetail();
    }).whenComplete(() {
  ...
  Future<void> _delBill() async {
    if (_billData == null) return;
    if (!PermissionUtils.checkPermission('014104', showTip: false)) {
      Toast.show('你无权删除移仓单，请在后台修改权限');
      return;
    }
    final confirm = await showDialog<bool>(
        context: context,
    ...
    if (confirm != true) return;
    setState(() => _submitAction = _MwAction.delete);
    request(HttpApi.stockRollDelBill, _billData).then((result) {
      if (!mounted) return;
      Toast.show('删除成功');
      Navigator.pop(context, true);
    }).whenComplete(() {
  ...
  Future<void> _print() async {
    if (_billData == null) return;
    setState(() => _submitAction = _MwAction.print);
    request(HttpApi.purchaseInstorePrint, {'menuid': '080401', 'data': _billData}).then((result) {
      if (!mounted) return;
      Toast.show('打印成功');
    }).whenComplete(() {
      if (mounted) setState(() => _submitAction = _MwAction.none);
      ...
      setState(() {
        for (final item in result) {
          ...
          if (existIdx >= 0) {
            ...
            final outcostprice = item['outcostprice']?.toString() ??
                item['outemptycostprice']?.toString() ??
                costprice;
            final incostprice = item['incostprice']?.toString() ??
                item['inemptycostprice']?.toString() ??
                costprice;
            final newItem = <String, dynamic>{
              ...
              'costprice': costprice,
              'outcostprice': outcostprice,
              'incostprice': incostprice,
      ...
      if (list.isNotEmpty) {
        ...
        if (_scanSettings.scanQtyMode == 0) {
          ...
          if (existingIndex >= 0) {
            ...
            // 不存在相同商品 → 直接新增一行
            setState(() {
              final costprice = prod['costprice']?.toString() ?? '0';
              final outcostprice = prod['outcostprice']?.toString() ??
                  prod['outemptycostprice']?.toString() ??
                  costprice;
              final incostprice = prod['incostprice']?.toString() ??
                  prod['inemptycostprice']?.toString() ??
                  costprice;
  ...
  // 移出仓库：选择后清空移入仓库
  Future<void> _selectOutWarehouse() async {
    FocusScope.of(context).unfocus();
  ...
  // 移入仓库：不能与移出仓库相同
  Future<void> _selectInWarehouse() async {
    FocusScope.of(context).unfocus();
    final result = await SelectWarehousePage.show(
      context,
      bsid: _bsid != null && _bsid!.isNotEmpty ? int.tryParse(_bsid!) : null,
    ...
    if (result != null && mounted) {
      final newCounterid = result['counterid']?.toString();
      if (newCounterid == _outcounterid) {
        Toast.show('移入仓库不能与移出仓库相同');
        return;
      }
      setState(() {
        _incounterid = newCounterid;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
          ...
          title: Text(_isEdit ? '修改移仓单' : '新增移仓单',
                      ...
                      child: CustomScrollView(
                        ...
                        slivers: [
                                  ...
                                  child: Container(
                                      ...
                                      child: Column(children: [
                                        ...
                                        _buildSelectRow(
                                            ...
                                            onTap: disabled ? null : _selectOutWarehouse,
                                            required: true),
                                        _buildDivider(),
                                        _buildSelectRow(
                                            label: '移入仓库',
                                            value: _incountername ?? '',
                                            onTap: disabled ? null : _selectInWarehouse,
                                            required: true),
                                        _buildDivider(),
                                        _buildSelectRow(
                                            label: '经手人',
                          ...
                          SliverPersistentHeader(
                              pinned: true, delegate: _MwStickyHeaderDelegate(state: this)),
                          if (_detailList.isEmpty)
                  ...
                  children: [
                    if (_detailList.isNotEmpty)
                      Padding(
                          ...
                          child: Text(
                              '移仓数量：${MathUtils.formatDecimal(1, _totalQty)}，共 ${_detailList.length} 项',
                              style: const TextStyle(fontSize: 13, color: Color(0xFF6B7280)))),
                    Row(children: [
                      if (_isEdit && _isSigned) ...[
                        Expanded(
                            child: GestureDetector(
                                onTap: _submitAction != _MwAction.none ? null : _print,
                                child: Container(
                                    height: 42,
                                    decoration: BoxDecoration(
                                    ...
                                    alignment: Alignment.center,
                                    child: _submitAction == _MwAction.print
                                        ? const SizedBox(
                                            width: 20,
                                            height: 20,
                        ...
                        if (!_isWithdrawPending && _bolHandleTT) ...[
                          Expanded(
                              child: GestureDetector(
                                  onTap: _submitAction != _MwAction.none ? null : _showMoreMenu,
                                  child: Container(
                                      height: 42,
                                      decoration: BoxDecoration(
                        ...
                        if (!_isRejected &&
                          ...
                          Expanded(
                            child: GestureDetector(
                                onTap: _submitAction != _MwAction.none ? null : () => _save(),
                                child: Container(
                                    height: 42,
                                    decoration: BoxDecoration(
                                        color: const Color(0xFF006EFF),
                                        borderRadius: BorderRadius.circular(6)),
                                    alignment: Alignment.center,
                                    child: _submitAction == _MwAction.save
                                        ? const SizedBox(
                                            width: 20,
                                            height: 20,
                                            child: CircularProgressIndicator(
                        ...
                        Expanded(
                            child: GestureDetector(
                                onTap: _submitAction != _MwAction.none
                                    ? null
                                    : () => _save(doSign: true),
                                child: Container(
                                    height: 42,
                                    decoration: BoxDecoration(
                                        ...
                                        borderRadius: BorderRadius.circular(6)),
                                    alignment: Alignment.center,
                                    child: _submitAction == _MwAction.sign
                                        ? const SizedBox(
                                            width: 20,
                                            height: 20,
                                            child: CircularProgressIndicator(
                        ...
                        if (_isRejected) ...[
                          ...
                          Expanded(
                              child: GestureDetector(
                                  onTap: _submitAction != _MwAction.none ? null : _restsign,
                                  child: Container(
                                      height: 42,
                                      decoration: BoxDecoration(
                                          ...
                                          borderRadius: BorderRadius.circular(6)),
                                      alignment: Alignment.center,
                                      child: _submitAction == _MwAction.withdraw
                                          ? const SizedBox(
                                              width: 20,
                                              height: 20,
                                              child: CircularProgressIndicator(
  ...
  void _showProductDetail(Map<String, dynamic> item, int index) {
    ...
    String editSupname = item['supname']?.toString() ?? '';
    String editStockqty = item['stockqty']?.toString() ?? item['stock']?.toString() ?? '';
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (disabled)
                          _buildDetailReadonlyField(
                              label: '移仓数量', value: editQty.toStringAsFixed(1))
                        else
                            ...
                            onTap: () async {
                              ...
                              if (result != null) {
                                setDialogState(() {
                                  ...
                                  editSupname = result['supname']?.toString() ?? '';
                                  // 同步更新批次对应的库存和成本价（对齐 lxAss batchHandle，无条件赋值）
                                  editStockqty =
                                      (double.tryParse(result['stockqty']?.toString() ?? '0') ?? 0)
                                          .toStringAsFixed(1);
                                  final batchCostprice =
                                      double.tryParse(result['costprice']?.toString() ?? '0') ?? 0;
                        ...
                        _buildDetailDivider(),
                        if (disabled || editBatchno.isNotEmpty)
                          _buildDetailReadonlyField(
                          ...
                          onPressed: disabled
                              ...
                              : () {
                                  setState(() {
                                    ...
                                    _detailList[index]['stockqty'] = editStockqty;
  ...
  Widget _buildDetailItem(Map<String, dynamic> item, int idx, bool disabled) {
    ...
    final outcostprice =
        double.tryParse(item['outcostprice']?.toString() ?? item['costprice']?.toString() ?? '') ??
            0;
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Expanded(
                ...
                Row(children: [
                  ...
                  Text('移出成本价：¥${MathUtils.formatDecimal(2, outcostprice)}',
                      textAlign: TextAlign.right,
                      style: const TextStyle(fontSize: 12, color: Color(0xFF7A7A7A))),
                ]),
                const SizedBox(height: 4),
                Text('移仓数量：${MathUtils.formatDecimal(1, qty)}',
                    style: const TextStyle(
...
class _MwStickyHeaderDelegate extends SliverPersistentHeaderDelegate {
  _MwStickyHeaderDelegate({required this.state});
  final _InventoryMovewarehouseAddPageState state;
  @override
  double get minExtent => state._isSigned
      ? _InventoryMovewarehouseAddPageState._stickySignedExtent
      : _InventoryMovewarehouseAddPageState._stickyMinExtent;
  @override
  double get maxExtent => state._isSigned
      ? _InventoryMovewarehouseAddPageState._stickySignedExtent
      : _InventoryMovewarehouseAddPageState._stickyMaxExtent;
  @override
  ...
  @override
  bool shouldRebuild(covariant _MwStickyHeaderDelegate oldDelegate) => true;
}
