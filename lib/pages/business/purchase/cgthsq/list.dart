...
class CgthsqListPage extends StatefulWidget {
  const CgthsqListPage({super.key});

  @override
  State<CgthsqListPage> createState() => _CgthsqListPageState();
}

class _CgthsqListPageState extends State<CgthsqListPage> with TickerProviderStateMixin {
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('退货申请单');
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('退货申请单', force: true);
    if (!mounted) return;
  ...
  void _startLoadData() {
    // 查看权限校验
    if (!PermissionUtils.checkPermission('012001', showTip: false)) {
      Toast.show('你无权查看退货申请，请在后台修改权限');
    } else {
      _loadData();
    }
  }
                    ...
                    child: _CgthsqCard(
                      ...
                      onTap: () {
                        ...
                        Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => CgthsqAddPage(billData: _list[index]),
                          ),
                        ).then((_) => _onRefresh());
                      },
                    ),
...
/// 列表卡片
class _CgthsqCard extends StatelessWidget {
  const _CgthsqCard({required this.item, required this.onTap});
