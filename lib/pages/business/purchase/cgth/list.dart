...
class PurchaseCgthListPage extends StatefulWidget {
  const PurchaseCgthListPage({super.key});

  @override
  State<PurchaseCgthListPage> createState() => _PurchaseCgthListPageState();
}

class _PurchaseCgthListPageState extends State<PurchaseCgthListPage>
    with TickerProviderStateMixin, LogPageMixin<PurchaseCgthListPage> {
  @override
  String get logPageName => '采购退货列表';
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('采购退货单');
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('采购退货单', force: true);
    if (!mounted) return;
  ...
  Future<void> _loadData() {
    ...
    return request(HttpApi.cgthFindList, {
      'is_page': 1,
      'cond': '',
      'field': 'createtime',
      'type': 'desc',
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildFilterRow(
                            label: '制单人',
                            value: tmpCreatename.isNotEmpty ? tmpCreatename : '所有制单人',
                            onTap: () async {
                              final result =
                      ...
                      GestureDetector(
                        onTap: () async {
                          ...
                          await Navigator.push<bool>(
                            context,
                            MaterialPageRoute(
                              builder: (_) => const PurchaseCgthAddPage(),
                            ),
                          );
                          _onRefresh();
                        },
                    ...
                    child: _CgthCard(
                      ...
                      onTap: () async {
                        ...
                        await Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => PurchaseCgthAddPage(billData: _list[index]),
                          ),
                        );
                        _onRefresh();
                      },
...
/// 列表卡片
class _CgthCard extends StatelessWidget {
  const _CgthCard({required this.item, required this.onTap});
