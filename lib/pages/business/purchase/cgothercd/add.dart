...
enum _COCdAction { none, save, sign, delete, retsign, print, withdraw, stop }

class CgothercdAddPage extends StatefulWidget {
  const CgothercdAddPage({super.key, this.billData});

  final Map<String, dynamic>? billData;

  @override
  State<CgothercdAddPage> createState() => _CgothercdAddPageState();
}

class _CgothercdAddPageState extends State<CgothercdAddPage> {
  ...
  /// 登录机构类型（对齐 Vue edit.vue isZyjmStore 取 userStore().store.storetype），
  ...
  _COCdAction _submitAction = _COCdAction.none;
  bool _detailLoading = false;
  bool _isTipNotMasterProduct = true; // 非供货资格商品校验标志
  // 进价校验放行标记（对齐 Vue cgothercd/edit.vue isTipInprice / isTipDoubleInprice，
  ...
  /// 是否为自营/加盟门店（storetype 1=自营, 2=加盟）—— 越库订单特有
  /// 对齐 Vue edit.vue：`isZyjmStore = computed(() => store.storetype == 1 || store.storetype == 2)`，
  /// 基于【登录机构】类型判断，而非所选收货机构/单据机构类型（_storetype），
  ...
  void initState() {
    ...
    // 加载登录机构类型（对齐 Vue edit.vue isZyjmStore 取 userStore().store.storetype）：
  ...
  void _loadNewModeDefaults() {
    ...
    // 未配置默认30天，对齐后台 cgothercd/edit.vue formDefault）
    int expireDay = 30;
    try {
      ...
      if (data is Map<String, dynamic>) {
        setState(() {
          ...
          for (final v in list) {
            ...
            // 越库订单特有字段：件数(jsqty) 和 赠送数量(presentqty)
            final packagenum = double.tryParse(c['packagenum']?.toString() ?? '') ?? 1;
            final qtyVal = double.tryParse(c['qty']?.toString() ?? '') ?? 0;
            final jsqtyVal = double.tryParse(c['jsqty']?.toString() ?? '') ??
                (packagenum > 0 ? qtyVal / packagenum : qtyVal);
            row.jsqtyController.text = MathUtils.formatDecimal(1, jsqtyVal);
            final presentqtyVal = double.tryParse(c['presentqty']?.toString() ?? '') ?? 0;
            row.presentqtyController.text = MathUtils.formatDecimal(1, presentqtyVal);
            row.priceController.text = MathUtils.formatDecimal(2, price);
            // 对齐 Vue handleProperty：amt 始终由 qty*price 重算
            row.amt = MathUtils.formatDecimalNum(3, MathUtils.mul(qtyVal, price));
  ...
  Future<void> _submit({bool withSign = false}) async {
    if (_isEdit) {
      if (!PermissionUtils.checkPermission('011603', showTip: false)) {
        Toast.show('你无权编辑越库订单，请在后台修改权限');
        return;
      }
    } else {
      if (!PermissionUtils.checkPermission('011602', showTip: false)) {
        Toast.show('你无权新增越库订单，请在后台修改权限');
        return;
      }
    }
    if (_storeid == null) {
    ...
    // 进价不一致 / 进价两倍差校验（对齐 Vue cgothercd/edit.vue save：
    ...
    setState(() => _submitAction = _COCdAction.save);
    ...
    if (_isEdit) {
      ...
      // 新增模式默认参数（对齐后台 cgothercd/edit.vue formDefault：
      // name/validdate/signflag/billtype:"2"/supselltype/fileLists）
      params = {
        'name': '',
        'signflag': 0,
        'billtype': '2',
    ...
    request(HttpApi.cgotherSave, params).then((result) {
      ...
      if (mounted) setState(() => _submitAction = _COCdAction.none);
    });
  }
  ...
  Future<void> _sign() async {
    if (!PermissionUtils.checkPermission('011605', showTip: false)) {
      Toast.show('你无权审核越库订单，请在后台修改权限');
      return;
    }
  ...
  void _doSign() {
    ...
    setState(() => _submitAction = _COCdAction.sign);
  ...
  Future<void> _retsign() async {
    ...
    setState(() => _submitAction = _COCdAction.retsign);
  ...
  Future<void> _restsign() async {
    if (!PermissionUtils.checkPermission('011606', showTip: false)) {
      Toast.show('你无权反审核越库订单，请在后台修改权限');
      return;
    }
    if (_billData == null) return;
    final confirm = await showDialog<bool>(
    ...
    setState(() => _submitAction = _COCdAction.withdraw);
    final params = Map<String, dynamic>.from(_billData!);
  ...
  Future<void> _print() async {
    if (_billData == null) return;
    setState(() => _submitAction = _COCdAction.print);
  ...
  Future<void> _stopBill() async {
    ...
    setState(() => _submitAction = _COCdAction.stop);
    final params = Map<String, dynamic>.from(_billData!);
    params['billtype'] = '2';
  ...
  Future<void> _delBill() async {
    if (!PermissionUtils.checkPermission('011604', showTip: false)) {
      Toast.show('你无权删除越库订单，请在后台修改权限');
      return;
    }
    if (_billData == null) return;
    final confirm = await showDialog<bool>(
    ...
    setState(() => _submitAction = _COCdAction.delete);
    final params = Map<String, dynamic>.from(_billData!);
    params['billtype'] = '2';
  ...
  /// 越库订单：件数合计
  String get _totalJsqty {
    double sum = 0;
    for (final row in _items) {
      sum += double.tryParse(row.jsqtyController.text) ?? 0;
    }
    return MathUtils.formatDecimal(1, sum);
  }
  ...
  List<Map<String, dynamic>> _buildSubmitDetailList() {
    ...
    return _items.map((row) {
      ...
      // 越库订单特有字段
      item['jsqty'] = MathUtils.formatDecimal(1, double.tryParse(row.jsqtyController.text) ?? 0);
      item['presentqty'] =
          MathUtils.formatDecimal(1, double.tryParse(row.presentqtyController.text) ?? 0);
      // oldprice 兜底处理（对齐 Vue cgothercd/edit.vue handleProperty：
      // 未审核 → inprice || oldprice；已审核 → oldprice || cgprice || 0），
      // 避免传 NULL 导致后端审核时 t_wms_receive_det.oldprice INSERT 失败
  ...
  Future<void> _refreshProductPrices() async {
    ...
    if (_billData != null) {
      ...
      params['billtypeid'] = _billData!['billtypeid']?.toString() ?? '0505';
    } else if (_newBillid != null && _newBillid!.isNotEmpty) {
      params['billid'] = _newBillid;
    }
    debugPrint(
        '[updateCgPrice-cgothercd] bsid=${params['bsid']}, supid=${params['supid']}, billid=${params['billid']}, items=${detaillist.length}');
  ...
  /// 构建商品查询参数（对齐 Vue mergDataFn - 越库 pstype=4, storeid 用配送中心）
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          _isEdit ? (_isSigned ? '越库订单详情' : (_isRejected ? '越库订单详情' : '修改越库订单')) : '新增越库订单',
          ...
          child: CustomScrollView(
            ...
            slivers: [
              ...
              SliverPersistentHeader(
                pinned: true,
                delegate: _CgothercdStickyHeaderDelegate(state: this),
              ),
  ...
  /// 对齐 PC 后台 cgothercd/edit.vue restsid：
      ...
      children: [
        SelectFieldItem(
          ...
          onTap: () async {
            ...
            // 对齐后台 cgothercd/edit.vue 收货机构 store-form-item merge-data：
        ...
        SelectFieldItem(
          ...
          onTap: () async {
            ...
            // 对齐后台 cgothercd/edit.vue 配送中心 store-form-item merge-data：
  ...
  /// 填充要货申请单数据（对齐 Vue getYhInfo — pstype=4）
  void _fillItemsFromRefbill(Map<String, dynamic> info, Map<String, dynamic> refResult) {
    setState(() {
      ...
      // 填充明细（对齐 Vue：filter pstype==4, 计算 jsqty, presentqty=0, camt）
      ...
      for (final v in list) {
        ...
        row.jsqtyController.text = MathUtils.formatDecimal(1, jsqty);
        row.presentqtyController.text = MathUtils.formatDecimal(1, 0);
        row.priceController.text = MathUtils.formatDecimal(2, price);
  ...
  Widget _buildBottomBar() {
    ...
    return Container(
      ...
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          // 统计栏：实时监听明细行数量/件数/价格输入，输入过程中同步刷新（对齐 instore）
          ListenableBuilder(
            ...
            builder: (context, _) {
              return Padding(
                ...
                child: Row(
                  children: [
                    Text(
                      '共${_items.length}项，合计数量：$_totalQty，合计件数：$_totalJsqty，总金额：$_totalAmt',
  ...
  Widget _buildEditUnsignedButtons() {
    final bool isLoading = _submitAction != _COCdAction.none;
    final buttons = <Widget>[];

    if (!_isWithdrawPending && _bolHandleTT) {
      // 对齐 Vue edit.vue 删除条件：signflag != 2 && bolHandleTT && !isZyjmStore
      final showDelete = !_isRejected && !_isZyjmStore;
      const showPrint = true;
      if (showDelete || showPrint) {
        buttons.add(PopupMenuButton<String>(
          ...
          itemBuilder: (ctx) => [
            if (showDelete) const PopupMenuItem(value: 'delete', child: Text('删除')),
            if (showPrint) const PopupMenuItem(value: 'print', child: Text('打印')),
          ],
          child: Container(
            height: 48,
    ...
    // 对齐 Vue edit.vue：保存不受 isZyjmStore 限制；审核额外要求 !isZyjmStore
    final showSave = !_isRejected &&
        !_isWithdrawPending &&
        (_bolHandleTT || (_reviewFlowUsers.isNotEmpty && _bolHandleT));
    final showAudit = showSave && !_isZyjmStore;
    if (showSave) {
      ...
      buttons.add(Expanded(
        child: OutlinedButton(
          ...
          child: _submitAction == _COCdAction.save
              ? const SizedBox(
                  width: 18,
                  height: 18,
    ...
    if (showAudit) {
      ...
      buttons.add(Expanded(
        child: ElevatedButton(
          ...
          child: _submitAction == _COCdAction.sign
              ? const SizedBox(
                  width: 18,
                  height: 18,
    ...
    if (_isRejected) {
      ...
      buttons.add(Expanded(
        child: ElevatedButton(
          ...
          child: _submitAction == _COCdAction.withdraw
              ? const SizedBox(
                  width: 18,
                  height: 18,
                  child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
  ...
  Widget _buildEditSignedButtons() {
    ...
    if (_bolHandleTTT && !_isZyjmStore) {
      buttons.add(Expanded(
        child: OutlinedButton(
          ...
          child: _submitAction == _COCdAction.retsign
              ? const SizedBox(
                  width: 18,
                  height: 18,
                  child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFF006EFF)))
    ...
    if (!_isZyjmStore) {
      ...
      buttons.add(Expanded(
        child: OutlinedButton(
          ...
          child: _submitAction == _COCdAction.stop
              ? const SizedBox(
                  width: 18,
                  height: 18,
    ...
    buttons.add(Expanded(
      child: ElevatedButton(
        ...
        child: _submitAction == _COCdAction.print
            ? const SizedBox(
                width: 18,
                height: 18,
                child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
  ...
  /// 新增模式：保存（始终显示）+ 审核（billSign && !isZyjmStore，对齐 Vue edit.vue）
  Widget _buildNewBillButtons() {
    ...
    // 对齐 Vue edit.vue：保存并审核 v-if="billSign && !isZyjmStore"
  ...
  List<Map<String, dynamic>> _buildSelectList() {
    return _items.map((row) {
      ...
      map['giftqty'] = double.tryParse(row.presentqtyController.text) ?? 0;
        ...
        for (final prod in result) {
          ...
          if (existing >= 0) {
            ...
            final row = _DetailRow()
              ...
              ..rawData = Map<String, dynamic>.from(prod)
              ..jsqtyController.text = MathUtils.formatDecimal(
                  1,
                  (double.tryParse((prod['qty'] ?? 1).toString()) ?? 1) /
                      (double.tryParse((prod['packagenum'] ?? 1).toString()) ?? 1))
              ..presentqtyController.text = MathUtils.formatDecimal(
                  1, double.tryParse(prod['giftqty']?.toString() ?? '0') ?? 0);
            _recalcRow(row);
      ...
      setState(() {
        if (existing >= 0) {
          ...
          final row = _DetailRow()
            ...
            ..rawData = Map<String, dynamic>.from(prod)
            ..jsqtyController.text = MathUtils.formatDecimal(
                1, qty / (double.tryParse((prod['packagenum'] ?? 1).toString()) ?? 1))
            ..presentqtyController.text = '0.0';
          _recalcRow(row);
          _items.insert(0, row);
        }
      });
...
// =================== 粘性表头代理 ===================
class _CgothercdStickyHeaderDelegate extends SliverPersistentHeaderDelegate {
  _CgothercdStickyHeaderDelegate({required this.state});
  final _CgothercdAddPageState state;

  @override
  double get minExtent => _CgothercdAddPageState._stickyMinExtent;

  @override
  double get maxExtent => (state._isSigned || !state._scanSettings.showInfraredInput)
      ? _CgothercdAddPageState._stickyMinExtent
      : _CgothercdAddPageState._stickyMaxExtent;
  ...
  @override
  bool shouldRebuild(covariant _CgothercdStickyHeaderDelegate oldDelegate) => true;
}
...
class _ProDetailSheet extends StatefulWidget {
  ...
  final double initialJsqty;
  final double initialPresentqty;

  /// 页面级上下文（对齐 Vue unit-form-item mergeData：bsid/billsupid，越库无 pstype）
...
class _ProDetailSheetState extends State<_ProDetailSheet> {
  ...
  late TextEditingController _presentqtyCtrl;
  ...
  void initState() {
    ...
    _jsqtyCtrl = TextEditingController(text: widget.initialJsqty.toStringAsFixed(1));
    _presentqtyCtrl = TextEditingController(text: widget.initialPresentqty.toStringAsFixed(1));
          ...
          Padding(
            ...
            child: Column(
              children: [
                ...
                const Divider(height: 1, color: Color(0xFFF3F4F6)),
                _buildFormField(label: '件数', controller: _jsqtyCtrl, isDecimal: true),
                const Divider(height: 1, color: Color(0xFFF3F4F6)),
                _buildFormField(label: '赠送数量', controller: _presentqtyCtrl, isDecimal: true),
                const Divider(height: 1, color: Color(0xFFF3F4F6)),
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      Navigator.pop(context, {
                        ...
                        'jsqty':
                            MathUtils.formatDecimal(1, double.tryParse(_jsqtyCtrl.text) ?? 0.0),
                        'presentqty': MathUtils.formatDecimal(
                            1, double.tryParse(_presentqtyCtrl.text) ?? 0.0),
                        'unit': _currentUnit,
                        'size': _currentSize,
  ...
  Future<void> _showExtendOptions(String type) async {
    ...
    // 对齐后台 cgothercd/edit.vue unit-form-item mergeData：
    // bsid 用收货机构（form.bsid）、billsupid、weekmonthflag:1（越库 unit/size 无 pstype）
    final params = <String, dynamic>{
      'productid': productid,
      'cgpriceflag': 1,
...
class _DetailRow {
  ...
  final jsqtyController = TextEditingController();
  final FocusNode jsqtyFocusNode = FocusNode();
  final presentqtyController = TextEditingController();
...
class _DetailItemState extends State<_DetailItem> {
  ...
  TextEditingController get _jsqtyCtrl => widget.row.jsqtyController;
  TextEditingController get _presentqtyCtrl => widget.row.presentqtyController;

  late final VoidCallback _onPriceBlur;
  late final VoidCallback _onQtyBlur;
  late final VoidCallback _onAmtBlur;
  late final VoidCallback _onJsqtyBlur;
  late final VoidCallback _onPresentqtyBlur;
  ...
  void initState() {
    ...
    _onJsqtyBlur = () => _onJsqtyChanged();
    _onPresentqtyBlur = () => _onPresentqtyChanged();
    ...
    widget.row.jsqtyFocusNode.addListener(_onJsqtyBlur);
    widget.row.presentqtyFocusNode.addListener(_onPresentqtyBlur);
    _syncAmtController();
  }
  ...
  void dispose() {
    ...
    widget.row.jsqtyFocusNode.removeListener(_onJsqtyBlur);
    widget.row.presentqtyFocusNode.removeListener(_onPresentqtyBlur);
    super.dispose();
  }
  ...
  void _formatAmtField() {
    ...
    if (price > 0) {
      final newQty = double.tryParse(MathUtils.formatDecimal(2, amt / price)) ?? 0;
      _qtyCtrl.text = MathUtils.formatDecimal(1, newQty);
      // 越库订单：联动计算 jsqty
      final packagenum = double.tryParse(widget.row.rawData?['packagenum']?.toString() ?? '') ?? 1;
      if (packagenum > 0) {
        widget.row.jsqtyController.text = MathUtils.formatDecimal(1, newQty / packagenum);
      }
    }
  ...
  void _onJsqtyChanged() {
    final jsqtyNode = widget.row.jsqtyFocusNode;
    if (jsqtyNode.hasFocus) return;
    _formatField(jsqtyNode, _jsqtyCtrl, 1);
    final jsqty = double.tryParse(_jsqtyCtrl.text) ?? 0;
    final packagenum = double.tryParse(widget.row.rawData?['packagenum']?.toString() ?? '') ?? 1;
    final qty = double.tryParse(MathUtils.formatDecimal(1, jsqty * packagenum)) ?? 0;
    _qtyCtrl.text = MathUtils.formatDecimal(1, qty);
    final price = double.tryParse(_priceCtrl.text) ?? 0;
    widget.row.amt = MathUtils.formatDecimalNum(3, MathUtils.mul(qty, price));
    _syncAmtController();
    final sellprice = double.tryParse(widget.row.rawData?['sellprice']?.toString() ?? '') ?? price;
    final presentqty = double.tryParse(_presentqtyCtrl.text) ?? 0;
    final raw = widget.row.rawData;
    if (raw != null) {
      raw['qty'] = qty;
      raw['jsqty'] = jsqty;
      raw['amt'] = widget.row.amt;
      raw['sellamt'] = MathUtils.formatDecimal(3, qty * sellprice + presentqty * sellprice);
    }
  }

  void _onPresentqtyChanged() {
    final presentqtyNode = widget.row.presentqtyFocusNode;
    if (presentqtyNode.hasFocus) return;
    _formatField(presentqtyNode, _presentqtyCtrl, 1);
  ...
  void _notifyChange() {
    final price =
        double.tryParse(MathUtils.formatDecimal(2, double.tryParse(_priceCtrl.text) ?? 0)) ?? 0;
    final qty =
        double.tryParse(MathUtils.formatDecimal(1, double.tryParse(_qtyCtrl.text) ?? 0)) ?? 0;
    widget.row.amt = MathUtils.formatDecimalNum(3, MathUtils.mul(qty, price));
    _syncAmtController();
    // 越库订单：联动计算 jsqty 和 sellamt
    final packagenum = double.tryParse(widget.row.rawData?['packagenum']?.toString() ?? '') ?? 1;
    if (packagenum > 0) {
      widget.row.jsqtyController.text = MathUtils.formatDecimal(1, qty / packagenum);
    }
    final sellprice = double.tryParse(widget.row.rawData?['sellprice']?.toString() ?? '') ?? price;
    final presentqty = double.tryParse(widget.row.presentqtyController.text) ?? 0;
    final sellamt = MathUtils.formatDecimal(3, qty * sellprice + presentqty * sellprice);
    final raw = widget.row.rawData;
    if (raw != null) {
      raw['price'] = price;
      raw['qty'] = qty;
  ...
  Future<void> _editDetail(BuildContext context) async {
    final result = await showModalBottomSheet<Map<String, dynamic>>(
      ...
      builder: (ctx) => AnimatedPadding(
        ...
        child: _ProDetailSheet(
          ...
          initialJsqty: double.tryParse(_jsqtyCtrl.text) ?? 0,
          initialPresentqty: double.tryParse(_presentqtyCtrl.text) ?? 0,
    ...
    if (result != null) {
      final qty = double.tryParse(
              MathUtils.formatDecimal(1, double.tryParse(result['qty']?.toString() ?? '') ?? 0)) ??
          0;
      final price = double.tryParse(MathUtils.formatDecimal(
              2, double.tryParse(result['price']?.toString() ?? '') ?? 0)) ??
          0;
      final jsqty = double.tryParse(MathUtils.formatDecimal(
              1, double.tryParse(result['jsqty']?.toString() ?? '') ?? 0)) ??
          0;
      final presentqty = double.tryParse(MathUtils.formatDecimal(
              1, double.tryParse(result['presentqty']?.toString() ?? '') ?? 0)) ??
          0;
      _qtyCtrl.text = MathUtils.formatDecimal(1, qty);
      _priceCtrl.text = MathUtils.formatDecimal(2, price);
      _jsqtyCtrl.text = MathUtils.formatDecimal(1, jsqty);
      _presentqtyCtrl.text = MathUtils.formatDecimal(1, presentqty);
      widget.row.amt = MathUtils.formatDecimalNum(3, MathUtils.mul(qty, price));
      _syncAmtController();
      if (widget.row.rawData != null) {
        ...
        widget.row.rawData!['jsqty'] = jsqty;
        widget.row.rawData!['presentqty'] = presentqty;
      }
    }
  }
          ...
          children: [
            ...
            Expanded(
              ...
              child: SizedBox(
                ...
                child: TextField(
                  ...
                  focusNode: widget.row.jsqtyFocusNode,
                  readOnly: widget.readOnly,
                  enabled: !widget.readOnly,
                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                  inputFormatters: [
                  ...
                  decoration: const InputDecoration(
                    isDense: true,
                    hintText: '件数',
                    hintStyle: TextStyle(fontSize: 10, color: Color(0xFF9CA3AF)),
            ...
            Expanded(
              ...
              child: SizedBox(
                height: 34,
                child: TextField(
                  controller: _presentqtyCtrl,
                  focusNode: widget.row.presentqtyFocusNode,
                  readOnly: widget.readOnly,
                  enabled: !widget.readOnly,
                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                  inputFormatters: [
