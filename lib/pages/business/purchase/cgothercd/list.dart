...
class CgothercdListPage extends StatefulWidget {
  const CgothercdListPage({super.key});

  @override
  State<CgothercdListPage> createState() => _CgothercdListPageState();
}

class _CgothercdListPageState extends State<CgothercdListPage> with TickerProviderStateMixin {
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('越库订单');
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('越库订单', force: true);
    if (!mounted) return;
  ...
  void _startLoadData() {
    // 查看权限校验
    if (!PermissionUtils.checkPermission('011601', showTip: false)) {
      Toast.show('你无权查看越库订单，请在后台修改权限');
    } else {
      _loadData();
    }
  }
  ...
  Future<void> _loadData() {
    ...
    return request(HttpApi.cgotherFindList, {
      'is_page': 1,
      'cond': '',
      'field': 'createtime',
      'type': 'desc',
                      ...
                      GestureDetector(
                        onTap: () async {
                          ...
                          await Navigator.push<bool>(
                            context,
                            MaterialPageRoute(
                              builder: (_) => const CgothercdAddPage(),
                            ),
                          );
                          _onRefresh();
                        },
                    ...
                    child: _CgothercdCard(
                      ...
                      onTap: () {
                        ...
                        Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => CgothercdAddPage(billData: _list[index]),
                          ),
                        ).then((_) => _onRefresh());
                      },
                    ),
...
/// 列表卡片
class _CgothercdCard extends StatelessWidget {
  const _CgothercdCard({required this.item, required this.onTap});
