...
class PurchaseCgorderListPage extends StatefulWidget {
  const PurchaseCgorderListPage({super.key});

  @override
  State<PurchaseCgorderListPage> createState() => _PurchaseCgorderListPageState();
}

class _PurchaseCgorderListPageState extends State<PurchaseCgorderListPage>
    with TickerProviderStateMixin, LogPageMixin<PurchaseCgorderListPage> {
  @override
  ...
  String _filterBuyerid = '';
  String _filterBuyername = '';
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('采购订货单');
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('采购订货单', force: true);
    if (!mounted) return;
  ...
  void _startLoadData() {
    // 查看权限校验
    if (!PermissionUtils.checkPermission('011301', showTip: false)) {
      Toast.show('你无权查看采购订货，请在后台修改权限');
    } else {
      _loadData();
    }
  }
  ...
  Future<void> _loadData() {
    ...
    return request(HttpApi.cgorderFindList, {
      'is_page': 1,
      'cond': '',
      'field': 'createtime',
      'type': 'desc',
  ...
  void _openFilterSheet() {
    ...
    String tmpBuyerid = _filterBuyerid;
    String tmpBuyername = _filterBuyername;
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildFilterRow(
                            label: '经手人',
                            value: tmpBuyername.isNotEmpty ? tmpBuyername : '全部经手人',
                            onTap: () async {
                              final result =
                                  await SelectBuyerPage.show(ctx, initialSelectedId: tmpBuyerid);
                              if (result != null) {
                                setSheetState(() {
                                  tmpBuyerid = result['buyerid']?.toString() ?? '';
                                  tmpBuyername = result['buyername']?.toString() ?? '';
                                });
                              }
                            },
                          ),
                          ...
                          child: GestureDetector(
                            onTap: () {
                              setState(() {
                                ...
                                _filterBuyerid = tmpBuyerid;
                                _filterBuyername = tmpBuyername;
                                _page = 1;
                                _list = [];
                                _hasMore = true;
                              });
                      ...
                      GestureDetector(
                        onTap: () async {
                          ...
                          await Navigator.push<bool>(
                            context,
                            MaterialPageRoute(
                              builder: (_) => const PurchaseCgorderAddPage(),
                            ),
                          );
                          _onRefresh();
                        },
                    ...
                    child: _CgorderCard(
                      ...
                      onTap: () async {
                        ...
                        await Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => PurchaseCgorderAddPage(billData: _list[index]),
                          ),
                        );
                        _onRefresh();
                      },
...
/// 列表卡片
class _CgorderCard extends StatelessWidget {
  const _CgorderCard({required this.item, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    final Color dhColor = _dhFlagColor(dhflagname);
                ...
                children: [
                  ...
                  if (dhflagname.isNotEmpty)
                    Container(
                      ...
                      decoration: BoxDecoration(
                        border: Border.all(color: dhColor),
