...
class CgotherListPage extends StatefulWidget {
  const CgotherListPage({super.key});

  @override
  State<CgotherListPage> createState() => _CgotherListPageState();
}

class _CgotherListPageState extends State<CgotherListPage> with TickerProviderStateMixin {
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('直配订单');
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('直配订单', force: true);
    if (!mounted) return;
  ...
  void _startLoadData() {
    // 查看权限校验
    if (!PermissionUtils.checkPermission('011501', showTip: false)) {
      Toast.show('你无权查看直配订单，请在后台修改权限');
    } else {
      _loadData();
    }
  }
                      ...
                      GestureDetector(
                        onTap: () async {
                          ...
                          await Navigator.push<bool>(
                            context,
                            MaterialPageRoute(
                              builder: (_) => const CgotherAddPage(),
                            ),
                          );
                          _onRefresh();
                        },
                    ...
                    child: _CgotherCard(
                      ...
                      onTap: () {
                        ...
                        Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => CgotherAddPage(billData: _list[index]),
                          ),
                        ).then((_) => _onRefresh());
                      },
                    ),
...
/// 列表卡片
class _CgotherCard extends StatelessWidget {
  const _CgotherCard({required this.item, required this.onTap});
