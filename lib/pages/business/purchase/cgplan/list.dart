...
class CgplanListPage extends StatefulWidget {
  const CgplanListPage({super.key});

  @override
  State<CgplanListPage> createState() => _CgplanListPageState();
}

class _CgplanListPageState extends State<CgplanListPage>
    with TickerProviderStateMixin, LogPageMixin<CgplanListPage> {
  @override
  String get logPageName => '采购计划列表';
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('采购计划单');
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('采购计划单', force: true);
    if (!mounted) return;
  ...
  void _startLoadData() {
    // 查看权限校验
    if (!PermissionUtils.checkPermission('011201', showTip: false)) {
      Toast.show('你无权查看采购计划，请在后台修改权限');
    } else {
      _loadData();
    }
  }
  ...
  Future<void> _loadData() {
    ...
    return request(HttpApi.cgplanFindList, {
      'is_page': 1,
      'cond': '',
      'field': 'createtime',
      'type': 'desc',
                    ...
                    child: _CgplanCard(
                      ...
                      onTap: () {
                        ...
                        Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => CgplanAddPage(billData: _list[index]),
                          ),
                        ).then((_) => _onRefresh());
                      },
                    ),
...
/// 列表卡片
class _CgplanCard extends StatelessWidget {
  const _CgplanCard({required this.item, required this.onTap});
