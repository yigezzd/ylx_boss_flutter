...
import 'package:flutter_deer/pages/business/chain/allot_entry/edit.dart';
...
/// 调拨入库单列表页（对齐 Vue chain/AllotEntry/AllotEntry.vue）
class AllotEntryListPage extends StatefulWidget {
  const AllotEntryListPage({super.key});

  @override
  State<AllotEntryListPage> createState() => _AllotEntryListPageState();
}

class _AllotEntryListPageState extends State<AllotEntryListPage>
    with TickerProviderStateMixin, LogPageMixin<AllotEntryListPage> {
  static const _quickLabels = ['昨天', '今天', '本周', '本月', '自定义'];

  @override
  String get logPageName => '调拨入库单列表';
  ...
  String _filterInsid = ''; // 调入机构
  String _filterInname = '';
  String _filterOutsid = ''; // 调出机构
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('调拨入库单');
    if (!mounted) return;
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('调拨入库单', force: true);
    if (!mounted) return;
    _applyTabCount(show ? 4 : 3);
  }

  /// 权限校验后加载列表数据（对齐 Vue permissionMsg('012908')）
  void _startLoadData() {
    if (!PermissionUtils.checkPermission('012908', showTip: false)) {
      Toast.show('你无权查看调拨入库单，请在后台修改权限');
    } else {
      _loadData();
    }
  }
                ...
                itemBuilder: (context, index) {
                  ...
                  return RepaintBoundary(
                    child: _EntryCard(
                      item: _list[index],
                      onTap: () {
                        // 查看权限校验（对齐 Vue permissionMsg('012908')）
                        if (!PermissionUtils.checkPermission('012908', showTip: false)) {
                          Toast.show('你无权查看调拨入库单，请在后台修改权限');
                          return;
                        }
                        ...
                        Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => AllotEntryEditPage(billData: _list[index]),
                          ),
                        ).then((_) => _onRefresh());
                      },
                    ),
...
/// 列表卡片（对齐 Vue AllotEntry 列表卡片，使用 insignflag）
class _EntryCard extends StatelessWidget {
  const _EntryCard({required this.item, required this.onTap});
  final Map<String, dynamic> item;
  final VoidCallback onTap;

  /// 对齐 Vue：insignflag 状态判断
  static Color _statusColor(dynamic insignflag) {
    final v = insignflag?.toString() ?? '';
    if (v == '1') return const Color(0xFF00A870);
    if (v == '2') return const Color(0xFFFF9900);
    if (v == '-1') return const Color(0xFFAAAAAA);
    return const Color(0xFFD54B5A);
  }

  static String _statusLabel(dynamic insignflag) {
    final v = insignflag?.toString() ?? '';
    if (v == '1') return '已审核';
    if (v == '2') return '已驳回';
    if (v == '-1') return '已作废';
  ...
  Widget build(BuildContext context) {
    ...
    final Color statusColor = _statusColor(item['insignflag']);
    final String statusLabel = _statusLabel(item['insignflag']);
