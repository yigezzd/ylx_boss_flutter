...
import 'package:flutter_deer/pages/business/chain/enquiry/edit.dart';
...
/// 要货申请单列表页（对齐 Vue chain/enquiry/enquiry.vue）
...
class _EnquiryListPageState extends State<EnquiryListPage>
    with TickerProviderStateMixin, LogPageMixin<EnquiryListPage> {
  static const _quickLabels = ['昨天', '今天', '本周', '本月', '自定义'];

  @override
  String get logPageName => '要货申请单列表';
  ...
  String _filterInid = ''; // 要货门店
  String _filterInname = '';
  String _filterOutid = ''; // 配送中心
  String _filterOutname = '';
  String _filterPhstatus = ''; // 配货状态
  String _filterDjstatus = ''; // 单据状态
  String _filterUrgentflag = ''; // 加急状态
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('要货申请单');
    if (!mounted) return;
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('要货申请单', force: true);
    if (!mounted) return;
    _applyTabCount(show ? 4 : 3);
  }

  void _startLoadData() {
    if (!PermissionUtils.checkPermission('013001', showTip: false)) {
      Toast.show('你无权查看要货申请单，请在后台修改权限');
    } else {
      _loadData();
    }
  }
  ...
  Future<void> _loadData() {
    ...
    return request(HttpApi.yhorderFindList, {
      ...
      if (_filterInid.isNotEmpty) 'insid': _filterInid,
      if (_filterOutid.isNotEmpty) 'outsid': _filterOutid,
      if (_filterPhstatus.isNotEmpty) 'phstatus': _filterPhstatus,
      if (_filterDjstatus.isNotEmpty) 'djstatus': _filterDjstatus,
      if (_filterUrgentflag.isNotEmpty) 'urgentflag': _filterUrgentflag,
    }).then((result) {
  ...
  void _openFilterSheet() {
    ...
    String tmpInid = _filterInid;
    String tmpInname = _filterInname;
    String tmpOutid = _filterOutid;
    String tmpOutname = _filterOutname;
    String tmpPhstatus = _filterPhstatus;
    String tmpDjstatus = _filterDjstatus;
    String tmpUrgentflag = _filterUrgentflag;
                          ...
                          child: GestureDetector(
                            onTap: () {
                              setState(() {
                                ...
                                _filterOutid = tmpOutid;
                                _filterOutname = tmpOutname;
                                _filterPhstatus = tmpPhstatus;
                                _filterDjstatus = tmpDjstatus;
                                _filterUrgentflag = tmpUrgentflag;
                                _page = 1;
                                _list = [];
                                _hasMore = true;
                              });
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 6),
                      // 新增按钮（对齐 Vue permissionMsg('013002')）
                      GestureDetector(
                        onTap: () async {
                          if (!PermissionUtils.checkPermission('013002', showTip: false)) {
                            Toast.show('你无权新增要货申请单，请在后台修改权限');
                            return;
                          }
                          logAdd();
                          await Navigator.push<bool>(
                            context,
                            MaterialPageRoute(
                              builder: (_) => const EnquiryEditPage(),
                            ),
                          );
                          _onRefresh();
                        },
                    ...
                    child: _EnquiryCard(
                      ...
                      onTap: () {
                        ...
                        Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => EnquiryEditPage(billData: _list[index]),
                          ),
                        ).then((_) => _onRefresh());
                      },
                    ),
...
class _EnquiryCard extends StatelessWidget {
  ...
  static String? _phstatusLabel(dynamic phstatus) {
    switch (phstatus?.toString()) {
      case '1':
        return '待配货';
      case '2':
        return '部分配货';
  ...
  /// 单据终止/过期标签（对齐 Vue enquiry.vue：djstatus 为中文状态值）
  static String? _djstatusLabel(String? djstatus) {
    if (djstatus == '已终止' || djstatus == '已过期') return djstatus;
    return null;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final bool isUrgent = item['urgentflag']?.toString() == '1';
    final String? phstatusLabel = _phstatusLabel(item['phstatus']);
    final String? djstatusLabel = _djstatusLabel(item['djstatus']?.toString());
                  ...
                  Expanded(
                    child: Row(
                      children: [
                        ...
                        // 已终止/已过期标签（对齐 Vue enquiry.vue 52-53 行角标）
                        if (djstatusLabel != null) ...[
                          const SizedBox(width: 6),
                          Container(
                            padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 1),
                            decoration: BoxDecoration(
                            ...
                            child: Text(djstatusLabel,
                                style: const TextStyle(fontSize: 10, color: Color(0xFFE02020))),
                          ),
                        ],
                      ],
                    ),
                ...
                children: [
                  ...
                  if (phstatusLabel != null)
                    Container(
                      ...
                      child: Text(phstatusLabel,
                          style: const TextStyle(fontSize: 10, color: Color(0xFF006EFF))),
                    ),
                ],
              ),
            ],
