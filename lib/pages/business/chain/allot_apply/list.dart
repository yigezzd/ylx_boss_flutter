...
import 'package:flutter_deer/pages/business/chain/allot_apply/edit.dart';
...
/// 调拨申请单列表页（对齐 Vue chain/AllotApply/AllotApply.vue）
class AllotApplyListPage extends StatefulWidget {
  const AllotApplyListPage({super.key});

  @override
  State<AllotApplyListPage> createState() => _AllotApplyListPageState();
}

class _AllotApplyListPageState extends State<AllotApplyListPage>
    with TickerProviderStateMixin, LogPageMixin<AllotApplyListPage> {
  static const _quickLabels = ['昨天', '今天', '本周', '本月', '自定义'];

  @override
  String get logPageName => '调拨申请单列表';
  ...
  String _filterInid = ''; // 调入机构
  String _filterInname = '';
  String _filterOutid = ''; // 调出机构
  ...
  void initState() {
    ...
    // 默认机构：当前登录机构（对齐 Vue sids: [store.id]）
    try {
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('调拨申请单');
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('调拨申请单', force: true);
    if (!mounted) return;
  ...
  /// 权限校验后加载列表数据
  void _startLoadData() {
    // 查看权限校验（对齐 Vue permissionMsg('012708')）
    if (!PermissionUtils.checkPermission('012708', showTip: false)) {
      Toast.show('你无权查看调拨申请单，请在后台修改权限');
    } else {
      _loadData();
    }
  }
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // ── 调出机构（对齐 Vue nosidsflag: 1, storetypes: [0,1,3]） ──
                          _buildFilterRow(
                            label: '调出机构',
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 6),
                      // 新增按钮（对齐 Vue permissionMsg('012702')）
                      GestureDetector(
                        onTap: () async {
                          if (!PermissionUtils.checkPermission('012702', showTip: false)) {
                            Toast.show('你无权新增调拨申请单，请在后台修改权限');
                            return;
                          }
                          logAdd();
                          await Navigator.push<bool>(
                            context,
                            MaterialPageRoute(
                              builder: (_) => const AllotApplyEditPage(),
                            ),
                          );
                          _onRefresh();
                        },
                    ...
                    child: _AllotCard(
                      ...
                      onTap: () {
                        ...
                        Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => AllotApplyEditPage(billData: _list[index]),
                          ),
                        ).then((_) => _onRefresh());
                      },
                    ),
...
/// 列表卡片
class _AllotCard extends StatelessWidget {
  const _AllotCard({required this.item, required this.onTap});
