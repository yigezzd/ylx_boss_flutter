...
import 'package:flutter_deer/pages/business/chain/returndeliver/edit.dart';
...
/// 配退发货单列表页（对齐 Vue chain/returnDeliver/returnDeliver.vue）
class ReturnDeliverListPage extends StatefulWidget {
  const ReturnDeliverListPage({super.key});

  @override
  State<ReturnDeliverListPage> createState() => _ReturnDeliverListPageState();
}

class _ReturnDeliverListPageState extends State<ReturnDeliverListPage>
    with TickerProviderStateMixin, LogPageMixin<ReturnDeliverListPage> {
  ...
  @override
  String get logPageName => '配退发货单列表';
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('配退发货单');
    if (!mounted) return;
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('配退发货单', force: true);
    if (!mounted) return;
    _applyTabCount(show ? 4 : 3);
  }

  /// 权限校验后加载列表数据（对齐 Vue permissionMsg('013408')）
  void _startLoadData() {
    if (!PermissionUtils.checkPermission('013408', showTip: false)) {
      Toast.show('你无权查看配退发货单，请在后台修改权限');
    } else {
      _loadData();
    }
  }
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 6),
                      // 新增按钮（对齐 Vue returnDeliverEdit 入口，权限 013402）
                      GestureDetector(
                        onTap: () async {
                          if (!PermissionUtils.checkPermission('013402', showTip: false)) {
                            Toast.show('你无权新增配退发货单，请在后台修改权限');
                            return;
                          }
                          logAdd();
                          await Navigator.push<bool>(
                            context,
                            MaterialPageRoute(
                              builder: (_) => const ReturnDeliverEditPage(),
                            ),
                          );
                          _onRefresh();
                        },
            ...
            : ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return RepaintBoundary(
                    child: _ReturnDeliverCard(
                      item: _list[index],
                      onTap: () {
                        // 查看权限校验（对齐 Vue permissionMsg('013408')）
                        if (!PermissionUtils.checkPermission('013408', showTip: false)) {
                          Toast.show('你无权查看配退发货单，请在后台修改权限');
                          return;
                        }
                        ...
                        Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => ReturnDeliverEditPage(billData: _list[index]),
                          ),
                        ).then((_) => _onRefresh());
                      },
                    ),
...
/// 列表卡片（对齐 Vue returnDeliver.vue 卡片布局）
class _ReturnDeliverCard extends StatelessWidget {
  const _ReturnDeliverCard({required this.item, required this.onTap});
  ...
  /// 配退状态标签（对齐 Vue dhflag 标签：0待处理/1待收货/2部分收货/3全部收货/4已终止）
  static String _dhflagLabel(String? dhflag) {
    if (dhflag == '0') return '待处理';
    if (dhflag == '1') return '待收货';
    if (dhflag == '2') return '部分收货';
    if (dhflag == '3') return '全部收货';
    if (dhflag == '4') return '已终止';
    return '';
  }
  ...
  Widget build(BuildContext context) {
    ...
    final String dhflagLabel = _dhflagLabel(item['dhflag']?.toString());
