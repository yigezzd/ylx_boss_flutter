...
import 'package:flutter_deer/pages/business/chain/deliver/edit.dart';
...
/// 配送发货单列表页（对齐 Vue chain/deliver/deliver.vue）
class DeliverListPage extends StatefulWidget {
  const DeliverListPage({super.key});

  @override
  State<DeliverListPage> createState() => _DeliverListPageState();
}

class _DeliverListPageState extends State<DeliverListPage>
    with TickerProviderStateMixin, LogPageMixin<DeliverListPage> {
  static const _quickLabels = ['昨天', '今天', '本周', '本月', '自定义'];

  @override
  String get logPageName => '配送发货单列表';
  ...
  String _filterInsid = ''; // 收货门店
  String _filterInname = '';
  String _filterOutsid = ''; // 配送中心
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('配送发货单');
    if (!mounted) return;
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('配送发货单', force: true);
    if (!mounted) return;
    _applyTabCount(show ? 4 : 3);
  }

  /// 权限校验后加载列表数据（对齐 Vue permissionMsg('013108')）
  void _startLoadData() {
    if (!PermissionUtils.checkPermission('013108', showTip: false)) {
      Toast.show('你无权查看配送发货单，请在后台修改权限');
    } else {
      _loadData();
    }
  }
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // ── 收货状态 ──
                          _buildDhflagSelector(
                              tmpDhflag, (v) => setSheetState(() => tmpDhflag = v)),
                          // ── 制单人 ──
                          _buildFilterRow(
                            label: '制单人',
  ...
  /// 收货状态选择器（对齐 Vue dhflagList）
  static Widget _buildDhflagSelector(String current, void Function(String) onChanged) {
    const items = [
      {'label': '全部', 'value': ''},
      {'label': '待处理', 'value': '0'},
      {'label': '待收货', 'value': '1'},
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 6),
                      // 新增按钮（对齐 Vue permissionMsg('013102')）
                      GestureDetector(
                        onTap: () async {
                          if (!PermissionUtils.checkPermission('013102', showTip: false)) {
                            Toast.show('你无权新增配送发货单，请在后台修改权限');
                            return;
                          }
                          logAdd();
                          await Navigator.push<bool>(
                            context,
                            MaterialPageRoute(
                              builder: (_) => const DeliverEditPage(),
                            ),
                          );
                          _onRefresh();
                        },
                ...
                itemBuilder: (context, index) {
                  ...
                  return RepaintBoundary(
                    child: _DeliverCard(
                      item: _list[index],
                      onTap: () {
                        // 查看权限校验（对齐 Vue permissionMsg('013108')）
                        if (!PermissionUtils.checkPermission('013108', showTip: false)) {
                          Toast.show('你无权查看配送发货单，请在后台修改权限');
                          return;
                        }
                        ...
                        Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => DeliverEditPage(billData: _list[index]),
                          ),
                        ).then((_) => _onRefresh());
                      },
                    ),
...
/// 列表卡片（对齐 Vue deliver.vue 卡片布局）
class _DeliverCard extends StatelessWidget {
  const _DeliverCard({required this.item, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    final String dhflagLabel = _dhflagLabel(dhflag);
