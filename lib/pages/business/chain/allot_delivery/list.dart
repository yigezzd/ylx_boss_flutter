...
import 'package:flutter_deer/pages/business/chain/allot_delivery/edit.dart';
...
/// 调拨出库单列表页（对齐 Vue chain/AllotDelivery/AllotDelivery.vue）
class AllotDeliveryListPage extends StatefulWidget {
  const AllotDeliveryListPage({super.key});

  @override
  State<AllotDeliveryListPage> createState() => _AllotDeliveryListPageState();
}

class _AllotDeliveryListPageState extends State<AllotDeliveryListPage>
    with TickerProviderStateMixin, LogPageMixin<AllotDeliveryListPage> {
  static const _quickLabels = ['昨天', '今天', '本周', '本月', '自定义'];

  @override
  String get logPageName => '调拨出库单列表';
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('调拨出库单');
    if (!mounted) return;
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('调拨出库单', force: true);
    if (!mounted) return;
    _applyTabCount(show ? 4 : 3);
  }

  /// 权限校验后加载列表数据（对齐 Vue permissionMsg('012808')）
  void _startLoadData() {
    if (!PermissionUtils.checkPermission('012808', showTip: false)) {
      Toast.show('你无权查看调拨出库单，请在后台修改权限');
    } else {
      _loadData();
    }
  }
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 6),
                      // 新增按钮（对齐 Vue permissionMsg('012802')）
                      GestureDetector(
                        onTap: () async {
                          if (!PermissionUtils.checkPermission('012802', showTip: false)) {
                            Toast.show('你无权新增调拨出库单，请在后台修改权限');
                            return;
                          }
                          logAdd();
                          await Navigator.push<bool>(
                            context,
                            MaterialPageRoute(
                              builder: (_) => const AllotDeliveryEditPage(),
                            ),
                          );
                          _onRefresh();
                        },
                ...
                itemBuilder: (context, index) {
                  ...
                  return RepaintBoundary(
                    child: _DeliveryCard(
                      item: _list[index],
                      onTap: () {
                        // 查看权限校验（对齐 Vue permissionMsg('012808')）
                        if (!PermissionUtils.checkPermission('012808', showTip: false)) {
                          Toast.show('你无权查看调拨出库单，请在后台修改权限');
                          return;
                        }
                        ...
                        Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => AllotDeliveryEditPage(billData: _list[index]),
                          ),
                        ).then((_) => _onRefresh());
                      },
                    ),
