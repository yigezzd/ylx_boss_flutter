...
class _ReturnTakeDeliveryEditPageState extends State<ReturnTakeDeliveryEditPage>
  ...
  void _loadDetail([Map<String, dynamic>? overrideParams]) {
    ...
    request(HttpApi.psrefundinGetInfo, params).then((result) {
      ...
      if (data is Map<String, dynamic>) {
        ...
        // 对齐 Vue getInfo：if (form.incounterid) handleCounterConfirm()
        if ((_incounterid ?? '').isNotEmpty) _handleCounterConfirm();
      }
    }).whenComplete(() {
  ...
  Future<void> _restsign() async {
    ...
    // 撤回仅需审核权限 013505，不要求保存权限，故跳过 _submit 内的保存权限校验
  ...
  /// 选择仓库/加载详情后刷新商品货架号（对齐 Vue handleCounterConfirm：dborder/updateLsPrice）
  void _handleCounterConfirm() {
  ...
  /// 收货仓库/送货员/备注可选；disabled 走只读区块合并展示）
  Widget _buildBillInfoEditable() {
    return Column(children: [
      // 配送中心/退货门店对齐 Vue jumpPge('insid'/'outsid')：仅 _readOnly（disabled）时锁定
      SelectFieldItem(
          label: '配送中心', required: true, value: _instorename ?? '', onTap: _selectInStore),
      const Divider(height: 1, color: Color(0xFFF3F4F6)),
      SelectFieldItem(
          label: '收货仓库', required: true, value: _incountername ?? '', onTap: _selectInCounter),
      const Divider(height: 1, color: Color(0xFFF3F4F6)),
      SelectFieldItem(
          label: '退货门店', required: true, value: _outstorename ?? '', onTap: _selectOutStore),
      const Divider(height: 1, color: Color(0xFFF3F4F6)),
      // 配退申请单/配退发货单仅展示（对齐 Vue disabled input）
      SelectFieldItem(label: '配退申请单号', value: _tpbillnoDisplay, enabled: false, onTap: () {}),
      const Divider(height: 1, color: Color(0xFFF3F4F6)),
      SelectFieldItem(label: '配退发货单号', value: _outbillnoDisplay, enabled: false, onTap: () {}),
      const Divider(height: 1, color: Color(0xFFF3F4F6)),
      SelectFieldItem(label: '送货员', value: _psname ?? '', onTap: _selectPsUser),
      const Divider(height: 1, color: Color(0xFFF3F4F6)),
