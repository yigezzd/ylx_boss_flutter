...
/// 配退申请单列表页（对齐 Vue chain/returnApplication/returnApplication.vue）
class ReturnApplicationListPage extends StatefulWidget {
  const ReturnApplicationListPage({super.key});

  @override
  State<ReturnApplicationListPage> createState() => _ReturnApplicationListPageState();
}

class _ReturnApplicationListPageState extends State<ReturnApplicationListPage>
    with TickerProviderStateMixin, LogPageMixin<ReturnApplicationListPage> {
  static const _quickLabels = ['昨天', '今天', '本周', '本月', '自定义'];

  /// 配退类型（对齐 Vue refundtypeList）
  static const _refundtypeLabels = ['全部', '配送差异', '退货'];
  static const _refundtypeValues = ['', '1', '2'];

  @override
  String get logPageName => '配退申请单列表';
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('配退申请单');
    if (!mounted) return;
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('配退申请单', force: true);
    if (!mounted) return;
    _applyTabCount(show ? 4 : 3);
  }

  /// 权限校验后加载列表数据（对齐 Vue permissionMsg('013308')）
  void _startLoadData() {
    if (!PermissionUtils.checkPermission('013308', showTip: false)) {
  ...
  Future<void> _loadData() {
    ...
    return request(HttpApi.psrefundapplyFindList, {
      ...
      if (_filterRefundtype.isNotEmpty) 'refundtype': _filterRefundtype,
  ...
  void _openFilterSheet() {
    ...
    int tmpRefundtypeIndex = _refundtypeValues.indexOf(_filterRefundtype);
    if (tmpRefundtypeIndex < 0) tmpRefundtypeIndex = 0;
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Row(
                              children: List.generate(_refundtypeLabels.length, (i) {
                                final selected = tmpRefundtypeIndex == i;
                                return Expanded(
                                  child: GestureDetector(
                                    onTap: () => setSheetState(() => tmpRefundtypeIndex = i),
                                    child: Container(
                                      ...
                                      child: Text(_refundtypeLabels[i],
                                          style: TextStyle(
                                            fontSize: 13,
                                            color:
                          ...
                          child: GestureDetector(
                            onTap: () {
                              setState(() {
                                ...
                                _filterRefundtype = _refundtypeValues[tmpRefundtypeIndex];
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 6),
                      // 新增按钮（对齐 Vue returnApplicationEdit 入口）
                      GestureDetector(
                        onTap: () async {
                          if (!PermissionUtils.checkPermission('013302', showTip: false)) {
                            Toast.show('你无权新增配退申请单，请在后台修改权限');
                            return;
                          }
                          logAdd();
                          await Navigator.push<bool>(
                            context,
                            MaterialPageRoute(
                              builder: (_) => const ReturnApplicationEditPage(),
                            ),
                          );
                          _onRefresh();
                        },
            ...
            : ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return RepaintBoundary(
                    child: _ReturnApplicationCard(
                      item: _list[index],
                      onTap: () {
                        // 查看权限校验（对齐 Vue permissionMsg('013308')）
                        if (!PermissionUtils.checkPermission('013308', showTip: false)) {
                          Toast.show('你无权查看配退申请单，请在后台修改权限');
                          return;
                        }
                        ...
                        Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => ReturnApplicationEditPage(billData: _list[index]),
                          ),
                        ).then((_) => _onRefresh());
                      },
                    ),
...
/// 列表卡片（对齐 Vue returnApplication.vue 卡片布局）
class _ReturnApplicationCard extends StatelessWidget {
  const _ReturnApplicationCard({required this.item, required this.onTap});
