...
/// 配送收货单列表页（对齐 Vue chain/receivingnote/receivingnote.vue）
class ReceivingnoteListPage extends StatefulWidget {
  const ReceivingnoteListPage({super.key});

  @override
  State<ReceivingnoteListPage> createState() => _ReceivingnoteListPageState();
}

class _ReceivingnoteListPageState extends State<ReceivingnoteListPage>
    with TickerProviderStateMixin, LogPageMixin<ReceivingnoteListPage> {
  static const _quickLabels = ['昨天', '今天', '本周', '本月', '自定义'];

  @override
  String get logPageName => '配送收货单列表';
  ...
  Future<void> _loadReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('配送收货单');
    if (!mounted) return;
  ...
  Future<void> _refreshReviewConfig() async {
    final show = await ReviewConfigUtils.shouldShowRejectedTab('配送收货单', force: true);
    if (!mounted) return;
    _applyTabCount(show ? 4 : 3);
  }

  /// 权限校验后加载列表数据（对齐 Vue permissionMsg('013208')）
  void _startLoadData() {
            ...
            : ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return RepaintBoundary(
                    child: _ReceivingnoteCard(
                      item: _list[index],
                      onTap: () {
                        // 查看权限校验（对齐 Vue permissionMsg('013208')）
                        if (!PermissionUtils.checkPermission('013208', showTip: false)) {
                          Toast.show('你无权查看配送收货单，请在后台修改权限');
                          return;
                        }
                        ...
                        Navigator.push<bool>(
                          context,
                          MaterialPageRoute(
                            builder: (_) => ReceivingnoteEditPage(billData: _list[index]),
                          ),
                        ).then((_) => _onRefresh());
                      },
                    ),
...
/// 列表卡片（对齐 Vue receivingnote.vue 卡片布局）
class _ReceivingnoteCard extends StatelessWidget {
  const _ReceivingnoteCard({required this.item, required this.onTap});
