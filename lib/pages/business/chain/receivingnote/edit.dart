...
enum _ReceivingnoteAction { none, save, sign, delete, retsign, print, withdraw }

/// 配送收货单编辑页（对齐 Vue chain/receivingnote/receivingnoteEdit.vue）
class ReceivingnoteEditPage extends StatefulWidget {
  const ReceivingnoteEditPage({super.key, this.billData});

  final Map<String, dynamic>? billData;

  @override
  State<ReceivingnoteEditPage> createState() => _ReceivingnoteEditPageState();
}

class _ReceivingnoteEditPageState extends State<ReceivingnoteEditPage>
    with LogPageMixin<ReceivingnoteEditPage> {
  @override
  String get logPageName => _isEdit ? '配送收货单详情' : '配送收货单新增';

  // ---- 表单字段（对齐 Vue query） ----
  String? _insid; // 收货门店
  String? _instorename;
  String? _incounterid; // 门店仓库
  ...
  String? _outbillno; // 配送单号（只读）
  DateTime? _pstime; // 收货时间（默认今天）
  ...
  _ReceivingnoteAction _submitAction = _ReceivingnoteAction.none;
  ...
  /// 对齐 Vue disabled：signflag==1 || signflag==2 || (!permission("013203") && billid)
  bool get _readOnly {
    if (_isSigned || _isRejected) return true;
    if (_isEdit && !PermissionUtils.checkPermission('013203', showTip: false)) return true;
    return false;
  }
  ...
  void _loadDetail([Map<String, dynamic>? overrideParams]) {
    ...
    request(HttpApi.psstockinGetInfo, params).then((result) {
      if (!mounted) return;
      ...
      if (data is Map<String, dynamic>) {
        setState(() {
          ...
          for (final v in list) {
            ...
            // 对齐 Vue getInfo → handleProperty：格式化 qty/price/amt 等（不覆盖 lspsprice）
            _applyWriteData(c);
  ...
  void _submit({bool withSign = false, bool skipSavePermissionCheck = false}) {
    // 撤回走 save(1) 时仅要求审核权限 013205（对齐 Vue restsign），跳过保存权限校验
    if (!skipSavePermissionCheck && !PermissionUtils.checkPermission('013203', showTip: false)) {
      Toast.show('你无权保存配送收货单，请在后台修改权限');
      return;
    }
    logSave(_isEdit ? '保存修改' : (withSign ? '保存并审核' : '保存单据'));

    // 对齐 Vue save 校验顺序：detaillist → insid → incounterid → outsid
    if (_items.isEmpty) {
      Toast.show('请选择商品');
      return;
    }
    ...
    setState(() => _submitAction = _ReceivingnoteAction.save);
    ...
    request(HttpApi.psstockinSave, params).then((result) {
      ...
      if (retData is Map<String, dynamic>) {
        ...
        if (withSign && PermissionUtils.checkPermission('013205', showTip: false)) {
  ...
  Future<void> _retsign() async {
    if (!PermissionUtils.checkPermission('013206', showTip: false)) {
      Toast.show('你无权反审核配送收货单，请在后台修改权限');
      return;
    }
    if (_billData == null) return;
    ...
    setState(() => _submitAction = _ReceivingnoteAction.retsign);
    final params = Map<String, dynamic>.from(_billData!);
  ...
  Future<void> _restsign() async {
    if (!PermissionUtils.checkPermission('013205', showTip: false)) {
      Toast.show('你无权审核配送收货单，请在后台修改权限');
      return;
    }
    if (_billData == null) return;
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('提示'),
        content: const Text('撤回单据后，所有审批步骤需重新处理！是否撤回该配送收货单？'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('取消')),
          TextButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('确定')),
        ],
  ...
  Future<void> _delBill() async {
    if (!PermissionUtils.checkPermission('013204', showTip: false)) {
      Toast.show('你无权删除配送收货单，请在后台修改权限');
      return;
    }
    if (_billData == null) return;
    final confirm = await showDialog<bool>(
    ...
    setState(() => _submitAction = _ReceivingnoteAction.delete);
    request(HttpApi.psstockinReject, {'billid': _billData!['billid']}).then((result) {
      if (!mounted) return;
      Toast.show('拒收成功');
  ...
  /// 打印单据（对齐 Vue menuid: '071401'）
  Future<void> _print() async {
    if (!PermissionUtils.checkPermission('013207', showTip: false)) {
      Toast.show('你无权打印配送收货单，请在后台修改权限');
      return;
    }
    if (_billData == null) return;
    setState(() => _submitAction = _ReceivingnoteAction.print);
    request(HttpApi.psstockinPrint, {
      'menuid': '071401',
      'data': _billData,
    }).then((result) {
      if (!mounted) return;
  ...
  /// 附件（对齐 Vue openAttach：menuid '071401'）
  ...
  // =================== 机构选择 ===================
  /// 选择收货门店（对齐 Vue store-form-item mergeData：{ stopflag: 0, storetypes: [1,2], psselecttype: 2, outsid } + isLs）
  Future<void> _selectInStore() async {
    ...
    if (result != null && mounted) {
      ...
      // 对齐 Vue selectStore2：收货门店不能与配送中心相同
      if ((_outsid ?? '').isNotEmpty && id == _outsid) {
        Toast.show('收货门店不能与配送中心相同');
        return;
      }
      setState(() {
        _insid = id;
        _instorename = result['storename']?.toString() ?? '';
        // 对齐 Vue selectStore2：清空门店仓库
        _incounterid = null;
        _incountername = null;
      });
    }
  ...
  /// 选择配送中心（对齐 Vue store-form-item mergeData：{ stopflag: 0, storetypes: [3], psselecttype: 2, insid, pscounterflag: spid==outsid?'1':'' } + isLs）
  Future<void> _selectOutStore() async {
    final result = await SelectStorePage.show(
      ...
      stopflag: '0',
      insid: _insid ?? '',
      pscounterflag: _spid != null && _outsid == _spid.toString() ? '1' : '',
      isLs: true,
    );
    if (result != null && mounted) {
      final id = result['storeid']?.toString() ?? '';
      // 对齐 Vue selectStore1：配送中心不能与收货门店相同
      if ((_insid ?? '').isNotEmpty && id == _insid) {
        Toast.show('收货门店不能与配送中心相同');
        return;
      }
      setState(() {
  ...
  Future<void> _selectInCounter() async {
    ...
    final result = await CommonSelectSheet.show(
      ...
      fetchData: (searchText, page) => request(HttpApi.counterGetList, {
        ...
        'nosidsflag': 1,
        'countertype': _insid == _spid.toString() ? 0 : '',
  ...
  void _updateLsPrice([Map<String, dynamic>? detail]) {
    ...
    request(HttpApi.dborderUpdateLsPrice, params).then((result) {
      ...
      setState(() {
        for (final row in _items) {
          final prodid = row.prodid ?? row.rawData?['productid']?.toString() ?? '';
          if (prodid.isEmpty) continue;
          for (final e in data) {
            if (e is Map && e['productid']?.toString() == prodid) {
              final shelves = e['shelves']?.toString() ?? '';
              row.rawData?['shelves'] = shelves;
              break;
            }
          }
        }
  ...
  Future<void> _selectProducts() async {
    // 对齐 Vue selectProductFn 校验顺序：insid → outsid
    if ((_insid ?? '').isEmpty) {
      Toast.show('请选择收货门店');
      return;
    }
    ...
    return Navigator.push<List<Map<String, dynamic>>>(
      ...
      MaterialPageRoute(
        builder: (_) => SelectProductPage(
          // 对齐 Vue selectProduct mergeData { insid, outsid, pspriceflag: 1 } + queryDefault（itemstatus/havestock）
          mergData: {
            'insid': _insid ?? '',
            'outsid': _outsid ?? '',
            'pspriceflag': 1,
  ...
  Future<void> _editScannedProduct(Map<String, dynamic> data) async {
    ...
    showModalBottomSheet<Map<String, dynamic>>(
      ...
      // 对齐 Vue unitConfirm 回填：换算率/售价/条码/自编码/货架号
      ...
      // 对齐 Vue writeData：重算 jsqty/amt/sellamt/diffqty
  ...
  // =================== 明细数据处理 ===================
  /// 对齐 Vue writeData/handleProperty：qty 变化时重算 jsqty/amt/sellamt/diffqty
  static void _applyWriteData(Map<String, dynamic> row) {
    ...
    // 对齐 Vue handleProperty：价格统一 2 位小数
    final price =
        MathUtils.formatDecimalNum(2, double.tryParse(row['price']?.toString() ?? '0') ?? 0);
    ...
    // 对齐 Vue writeData：amt = qty * price（配送价），金额 3 位小数
    row['amt'] = MathUtils.formatDecimal(3, MathUtils.mul(qty, price));
    // 对齐 Vue writeData：diffqty = yhqty - qty
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          _isEdit ? (_isSigned || _isRejected ? '配送收货单详情' : '修改配送收货单') : '新增配送收货单',
          style:
  ...
  /// 只读单据信息（对齐 Vue disabled：收货门店合并显示 门店-仓库）
  Widget _buildBillInfoReadonly() {
    return Column(children: [
      _buildReadonlyField(label: '收货门店', value: '${_instorename ?? ''}-${_incountername ?? ''}'),
      const Divider(height: 1, color: Color(0xFFF3F4F6)),
      _buildReadonlyField(label: '配送中心', value: _outstorename ?? ''),
      const Divider(height: 1, color: Color(0xFFF3F4F6)),
      _buildReadonlyField(label: '收货时间', value: _pstimeController.text),
      const Divider(height: 1, color: Color(0xFFF3F4F6)),
      _buildReadonlyField(label: '原单号', value: (_yhbillno ?? '').isEmpty ? '无' : _yhbillno!),
      const Divider(height: 1, color: Color(0xFFF3F4F6)),
  ...
  /// 待审核状态按钮（更多[删除=拒收+打印] + 保存 + 审核）
  Widget _buildEditUnsignedButtons() {
    final isLoading = _submitAction != _ReceivingnoteAction.none;
    ...
    if (showSA) {
      ...
      buttons.add(Expanded(
          child: OutlinedButton(
              ...
              child: _submitAction == _ReceivingnoteAction.save
                  ? const SizedBox(
                      width: 18,
                      height: 18,
      ...
      buttons.add(Expanded(
          child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                  ...
                  elevation: 0,
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
              child: _submitAction == _ReceivingnoteAction.sign
                  ? const SizedBox(
                      width: 18,
                      height: 18,
    ...
    b.add(Expanded(
        child: OutlinedButton(
            ...
            style: OutlinedButton.styleFrom(
                ...
                padding: const EdgeInsets.symmetric(vertical: 14),
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
            child: _submitAction == _ReceivingnoteAction.delete
                ? const SizedBox(
                    width: 18,
                    height: 18,
    ...
    b.add(Expanded(
        child: ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
                ...
                elevation: 0,
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
            child: _submitAction == _ReceivingnoteAction.print
                ? const SizedBox(
                    width: 18,
                    height: 18,
                    child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
    ...
    b.add(Expanded(
        child: OutlinedButton(
            ...
            style: OutlinedButton.styleFrom(
                ...
                padding: const EdgeInsets.symmetric(vertical: 14),
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
            child: _submitAction == _ReceivingnoteAction.withdraw
                ? const SizedBox(
                    width: 18,
                    height: 18,
      ...
      b.add(Expanded(
          child: OutlinedButton(
              ...
              style: OutlinedButton.styleFrom(
                  ...
                  padding: const EdgeInsets.symmetric(vertical: 14),
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
              child: _submitAction == _ReceivingnoteAction.retsign
                  ? const SizedBox(
                      width: 18,
                      height: 18,
                      child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFF006EFF)))
  ...
  /// 新增状态按钮（保存 013203 + 审核 013205）
  Widget _buildNewBillButtons() {
    final isLoading = _submitAction != _ReceivingnoteAction.none;
    final canSave = PermissionUtils.checkPermission('013203', showTip: false);
    final canSign = PermissionUtils.checkPermission('013205', showTip: false);
    final b = <Widget>[];
    b.add(Expanded(
        child: ElevatedButton(
...
// =================== 明细卡片（对齐 Vue 商品列表卡片布局：名称+价格+批次+数量+金额） ===================
  ...
  void _editDetail() {
    ...
    showModalBottomSheet<Map<String, dynamic>>(
      ...
      if (raw != null) {
        ...
        // 对齐 Vue handleBatchConfirm：批次带回的库存量
        final outstockqty = result['outstockqty']?.toString() ?? '';
        if (outstockqty.isNotEmpty) raw['outstockqty'] = outstockqty;
        // 对齐 Vue writeData：重算 jsqty/amt/sellamt/diffqty
        _ReceivingnoteEditPageState._applyWriteData(raw);
...
// =================== 商品明细编辑弹窗（对齐 Vue proDetails：价格=配送价） ===================
...
class _ProDetailSheetState extends State<_ProDetailSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final psprice = data['lspsprice']?.toString() ??
        ...
        data['price']?.toString() ??
        '';
    // 收货门店库存 / 配送中心库存（批次选择后配送中心库存以选中的批次库存为准）
    ...
    setState(() {
      if (type == 'unit') {
        ...
        if (lspsprice != null && lspsprice.isNotEmpty) {
          ...
          widget.productData['lspsprice'] = lspsprice;
        }
        // 对齐 Vue unitConfirm：unitonlyid 非基本单位时 packagenum = 1，否则取扩展行换算率
        widget.productData['packagenum'] = uid.isNotEmpty
            ? 1
            : (r['packagenum']?.toString() ?? widget.productData['packagenum'] ?? 1);
        final sp = r['sellprice']?.toString();
        if (sp != null) widget.productData['sellprice'] = sp;
